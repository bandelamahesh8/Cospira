...
const initRedis = async (attempt = 1) => {
    if (process.env.USE_REDIS === 'false') {
        console.log('Redis disabled (USE_REDIS=false), using in-memory storage');
        if (process.env.NODE_ENV === 'production') {
            console.error('❌ PRODUCTION ERROR: Redis cannot be disabled in production.');
            process.exit(1);
        }
        return;
    }
...
export const setCache = async (key, value, ttlSeconds) => {
    ...
    // No fallback for cache in local mode
};
