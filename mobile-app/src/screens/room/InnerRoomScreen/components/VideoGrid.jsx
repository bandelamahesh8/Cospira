...
const VideoGrid = ({
  ...
  const getVideoOnlyStream = (key, raw) => {
    ...
    if (cached && cached.trackId === trackId && cached.stream) {
      ...
      // Only return cached stream if the track is still healthy
      if (cachedTrack && cachedTrack.readyState !== 'ended') {
  ...
  const localDisplayStream = localStream;
  ...
  const participants = useMemo(() => {
    if (isPip) {
      ...
      const remoteParticipants = filteredUsers.map((user) => {
        ...
        // Pass RAW stream for remote - new MediaStream() breaks native toURL() on react-native-webrtc.
        const stream = raw || null;
        ...
        return (
          ...
          >
            {pageParticipants.map((p) => {
              const videoId = p.stream?.getVideoTracks?.()?.[0]?.id || 'no-video';
              const tileKey = `tile-${p.id}-${videoId}`;
