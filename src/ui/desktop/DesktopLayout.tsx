...
import { Trophy, TrendingUp, Users, Palette, Crown, Sliders, Calendar, Settings, Bell, Brain } from 'lucide-react';
...
export const DesktopLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [view, setView] = React.useState<'desktop' | 'shop' | 'tournament' | 'clans' | 'identity' | 'meta' | 'admin' | 'seasons' | 'brain'>('desktop');
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-background overflow-hidden">
      ...
      >
        <span className="text-xs font-semibold text-muted-foreground/70 tracking-widest uppercase">Cospira Desktop</span>
        <div className="flex items-center gap-3">
             {/* Connection Health Indicator */}
             <div className="flex items-center gap-1.5" title={isConnected ? "Connected" : "Disconnected"}>
                <Wifi className={cn("w-3 h-3 transition-colors", isConnected ? "text-emerald-500" : "text-destructive")} />
             </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 relative overflow-auto">
          {view === 'shop' ? <Shop /> : 
           view === 'tournament' ? <TournamentLobby /> :

           view === 'clans' ? <ClanManager /> :
           view === 'identity' ? <IdentityEditor /> :
           view === 'meta' ? <MetaLeaderboard /> :
           view === 'admin' ? <BalanceDashboard /> :
           view === 'seasons' ? <SeasonHub /> :
           view === 'brain' ? <BrainDashboard /> :
           children}
        </main>
