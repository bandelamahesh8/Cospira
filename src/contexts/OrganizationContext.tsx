...
import { Organization, PermissionKey, OrgMode } from '@/types/organization';
...
export const OrganizationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const deleteOrganization = async (orgId: string) => {
    try {
      ...
      // If the deleted org was the current one, select another or none
  ...
  const removeMember = async (_userId: string) => {
    ...
    if (!hasPermission('MEMBER_REMOVE')) {
      toast.error('Permission Denied', {
        description: 'You do not have rights to remove members.',
      });
      return;
    }
