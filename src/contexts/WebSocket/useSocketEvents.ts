...
import { User, Message, FileData, RoomJoinedData, SfuNewProducerData, GameState, RoomInfo } from '@/types/websocket';
import { RoomMode, getModeConfig } from '@/services/RoomIntelligence';
...
export const useSocketEvents = ({
    ...
    useEffect(() => {
        ...
        const onRoomJoined = async (data: RoomJoinedData) => {
            ...
            setState((prev) => ({
                ...
                roomModeConfig: getModeConfig((data.settings?.mode || data.mode || data.roomMode || 'mixed') as RoomMode),
            }));
            ...
            try {
                ...
                try {
                    ...
                    logger.warn('Failed to fetch ICE servers, using defaults:', error);
                }
        ...
        const onNewMessage = (message: Message) => {
            setState((prev) => {
                ...
                if (message.userId === currentUserId) {
                    const exists = prev.messages.some(m => m.id === message.id || (m.pending && m.content === message.content));
                    if (exists) {
                        // Replace the pending message with the official server version
                        const filtered = prev.messages.filter(m => m.id !== message.id && (!m.pending || m.content !== message.content));
            ...
            if (message.userId !== currentUserId && message.userId !== 'system') {
                 getDesktopAdapter().showNotification('New Message', `${message.userName}: ${message.content}`);
            }
        };
        ...
        const onUserStatusChange = (data: { userId: string; status: 'online' | 'away' }) => {
            setState((prev) => ({
                ...prev,
                users: prev.users.map((u) => u.id === data.userId ? { ...u, status: data.status, isAway: data.status === 'away' } : u)
            }));
        };
        ...
        const onUserVideoChange = (data: { userId: string; enabled: boolean }) => {
            setState((prev) => ({
                ...prev,
                users: prev.users.map((u) => u.id === data.userId ? { ...u, isVideoOn: data.enabled } : u)
            }));
        };
        ...
        const onYoutubeStarted = ({ videoId, presenterName }: { videoId: string; presenterName: string }) => {
            ...
            toast({ title: 'YouTube Started', description: `${presenterName || 'Someone'} started a video.` });
        };
        ...
        const onFilePresented = ({ fileData, presenterName }: { fileData: FileData; presenterName: string }) => {
            if (!fileData) return;
            setState((prev) => ({
                ...prev,
            ...
            toast({ title: 'File Presented', description: `${presenterName || 'Someone'} is presenting.` });
        };
        ...
        const onGameStrike = ({ player, strikes }: { player: string; strikes: number }) => {
            setState((prev) => {
                ...
                return {
                    ...prev,
                    gameState: prev.gameState ? { ...prev.gameState, players: updatedPlayers } : prev.gameState,
                };
            });
        ...
        const onGameEnded = (gameState: GameState) => {
            ...
            if (gameState.winner) {
                 ...
                 toast({ title: 'Game Over', description: gameState.winner === 'draw' ? 'Draw!' : `${winnerName} won!` });
            }
        };

        const onSystemAnnouncement = (data: { type: 'success' | 'warning' | 'danger' | 'info'; message: string }) => {
            toast({
        ...
        const onTranscript = (data: { userId: string; text: string; timestamp: number; isFinal: boolean }) => {
             setState((prev) => {
        ...
        const onTimerStarted = (data: { duration: number; startedAt: number; label: string }) => {
            setState((prev) => ({ ...prev, activeTimer: data }));
            toast({ title: 'Timer Started', description: `Timer for ${data.duration} mins: ${data.label}` });
        };

        const onPollCreated = (data: { id: string; question: string; options: string[]; expiresAt: number }) => {
        ...
        const onBrowserCommandResult = (data: { command: string; action: unknown; result: { success: boolean, message?: string } }) => {
            if (data.result.success) {
                toast({ title: 'Assistant Action', description: `Executed: ${data.command}` });
            } else {
                toast({ title: 'Assistant Failed', description: data.result.message || `Failed: ${data.command}`, variant: 'destructive' });
            }
        };
    ...
    }, [navigate, signalingRef, sfuManagerRef, setState, signOut, currentUserId]);
};
