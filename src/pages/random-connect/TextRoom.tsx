...
const TextRoom = () => {
  ...
  const { socket, messages, users, files, isConnected } = useWebSocket();
            ...
            <div className='flex items-center gap-3 mb-6'>
              ...
              <div>
                <h1 className='font-black text-lg tracking-tight uppercase'>
                  Text<span className='text-indigo-500'>Mode</span>
                </h1>
              ...
              <div className='p-4 rounded-xl bg-white/5 border border-white/5'>
                ...
                <div className='flex -space-x-2'>
                  {users.map((u, i) => (
                    <div
                      key={i}
                      className='w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-xs font-bold ring-2 ring-[#0A0A0A]'
                    >
                      {u.name?.[0] || 'A'}
                    </div>
                  ))}
