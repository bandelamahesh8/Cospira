...
const RandomLanding = () => {
    ...
    const { mode = 'video', intent = 'casual', interests = [], preVerified = false } = location.state || {};
    ...
    const handleProceedSearch = async () => {
        ...
        try {
            // Hand over the existing preview stream to save time!
            if (previewStream) {
                 isHandingOverRef.current = true; // Set flag to prevent cleanup
                 await enableMedia(previewStream);
    ...
    // NEW: Leave handler
    const handleLeave = () => {
        const confirmed = window.confirm("Are you sure you want to leave the matching session? This will disconnect your audio and video.");
        if (!confirmed) return;
    ...
    useEffect(() => {
        ...
        return () => { socket.off('match_found', onMatch); };
    }, [socket, navigate]);
            ...
            <div className="h-screen w-full flex flex-col font-sans overflow-hidden relative">
                    ...
                    {mediaWarning && (
                        ...
                        >
                            <div className="w-full max-w-md bg-[#0A0D11] border border-red-500/30 rounded-3xl p-8 text-center space-y-6 shadow-2xl">
                                ...
                                <div>
                                    <h2 className="text-2xl font-black text-white uppercase tracking-tight mb-2">Media Signal Lost</h2>
                                    <p className="text-red-400 font-medium tracking-wide">Camera and Microphone Access Required</p>
                                    <p className="text-white/40 text-sm mt-3 leading-relaxed">System protocols require active media streams for global matching. Please re-enable your hardware.</p>
                                </div>
                        ...
                        >
                            <div className="w-full max-w-md bg-[#0A0D11] border border-white/10 rounded-3xl p-8 text-center space-y-6">
                                <div className="w-16 h-16 bg-indigo-500/10 rounded-full flex items-center justify-center mx-auto mb-4 animate-pulse">
                                    {isCheckingPermissions ? <Loader2 className="w-8 h-8 text-indigo-500 animate-spin" /> : 
                                     permissionGranted ? <CheckCircle className="w-8 h-8 text-emerald-500" /> : 
                                     <AlertTriangle className="w-8 h-8 text-amber-500" />}
                                </div>
                                <h2 className="text-2xl font-black text-white uppercase tracking-tight">System Check</h2>
                                {permissionGranted ? (
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <button id="p-retry-btn" onClick={() => navigate(0)} className="px-6 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-white font-bold text-xs uppercase">Retry Check</button>
                                    </div>
                                )}
                            </div>
                        </motion.div>
                        ...
                        >
                             <div className="w-full max-w-md bg-[#0A0D11] border border-red-500/20 rounded-3xl p-8 space-y-8 relative overflow-hidden">
                                ...
                                <div className="text-center space-y-2">
                                    ...
                                    <h2 className="text-2xl font-black text-white uppercase tracking-tight">Restricted Access</h2>
                                ...
                                <div className="space-y-4 bg-red-500/5 p-6 rounded-2xl border border-red-500/10">
                                    <div className="flex items-start gap-4">
                                        <Checkbox id="age" checked={ageConfirmed} onCheckedChange={(c) => setAgeConfirmed(c === true)} className="mt-1 border-red-500/50 data-[state=checked]:bg-red-500 data-[state=checked]:text-white" />
                                        <label htmlFor="age" className="text-sm font-bold text-white uppercase tracking-wide leading-none cursor-pointer">I confirm I am 18+ years old</label>
                                    </div>
                                    <div className="flex items-start gap-4">
                                        <Checkbox id="terms" checked={termsConfirmed} onCheckedChange={(c) => setTermsConfirmed(c === true)} className="mt-1 border-red-500/50 data-[state=checked]:bg-red-500 data-[state=checked]:text-white" />
                                        <label htmlFor="terms" className="text-sm font-bold text-white uppercase tracking-wide leading-none cursor-pointer">I accept the Terms & Conditions</label>
                                    </div>
                                </div>
                                <div className="flex gap-4">
                                    <button id="age-cancel-btn" onClick={handleLeave} className="flex-1 h-12 bg-transparent hover:bg-white/5 border border-white/10 rounded-xl text-white/40 hover:text-white font-bold text-xs uppercase transition-colors">Cancel</button>
                                    <button id="age-confirm-btn" onClick={handleConfirmLegal} disabled={!ageConfirmed || !termsConfirmed} className="flex-1 h-12 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white rounded-xl font-black uppercase tracking-widest transition-all">Enter</button>
                                </div>
                             </div>
                        </motion.div>
                    )}
                ...
                <header className="h-16 bg-[#0A0D11]/80 backdrop-blur-md border-b border-white/5 px-6 flex items-center justify-between shrink-0 z-20">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xl font-black text-white tracking-tight">COSPIRA<span className="text-indigo-500">.LIVE</span></span>
                    </div>
                    <div className="flex items-center gap-4">
                         <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-full border border-white/5">
                            ...
                            <span className="text-xs font-bold text-white/60 uppercase tracking-wider">{Object.keys(users).length + 4000} Online</span>
                         </div>
                    </div>
                </header>
                        ...
                        <div className="flex-1 bg-[#111] rounded-2xl overflow-hidden relative shadow-2xl border border-white/5 group ring-1 ring-white/5">
                            {state === 'connected' && Array.from(remoteStreams.values())[0] ? (
                                 <video 
                                     ...
                                     ref={ref => { if (ref) ref.srcObject = Array.from(remoteStreams.values())[0]; }} 
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0A0D11]">
                                    ...
                                    <h3 className="text-2xl font-black text-white uppercase tracking-wider animate-pulse">Scanning Sector...</h3>
                                    <p className="text-indigo-400/60 font-mono text-sm mt-3 tracking-widest">CONNECTING TO PARTNER</p>
                                </div>
                            ) : (
                                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0A0D11] bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]">
                                    ...
                                    <h3 className="text-xl font-bold text-white mb-2 tracking-wide">Waiting for Command</h3>
                                    <p className="text-white/30 tracking-widest text-sm uppercase">Click Start to Begin</p>
                                </div>
                            )}
                            ...
                            <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg flex items-center gap-2 border border-white/10">
                                 <span className={`w-2 h-2 rounded-full ${state === 'connected' ? 'bg-indigo-500 animate-pulse' : 'bg-gray-500'}`} />
                                 <span className="text-white font-bold text-[10px] tracking-widest uppercase">REMOTE FEED</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 bg-[#111] rounded-2xl overflow-hidden relative shadow-2xl border border-white/5 ring-1 ring-white/5">
                            {mode === 'video' ? (
                                ...
                                <div className="absolute inset-0 flex items-center justify-center bg-[#1a1d21]">
                                    <p className="text-white/30 font-bold uppercase tracking-widest">Camera Disabled</p>
                                </div>
                            )}

                            <div className="absolute bottom-4 left-4 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-lg flex items-center gap-2 border border-white/10">
                                <span className="w-2 h-2 rounded-full bg-emerald-500" />
                                 <span className="text-white font-bold text-[10px] tracking-widest uppercase">YOU</span>
                            </div>

                             <div className="absolute top-4 right-4 flex gap-2">
                                 <NeuralInformer title="Mic Interface" description="Control your voice broadcast status. Signals are processed through low-latency neural buffers.">
                                    <button id="t-audio-btn" onClick={toggleAudio} className={`p-2.5 rounded-xl backdrop-blur-md border border-white/10 transition-all ${isAudioEnabled ? 'bg-black/40 text-white hover:bg-black/60' : 'bg-red-500 text-white shadow-lg shadow-red-500/20'}`}>
                                        {isAudioEnabled ? <Mic size={18} /> : <MicOff size={18} />}
                                    </button>
                                 </NeuralInformer>
                                {mode === 'video' && (
                                    <NeuralInformer title="Video Capture" description="Toggle active optical transmission. Protocols require active streams for continuous sector alignment.">
                                        <button id="t-video-btn" onClick={toggleVideo} className={`p-2.5 rounded-xl backdrop-blur-md border border-white/10 transition-all ${isVideoEnabled ? 'bg-black/40 text-white hover:bg-black/60' : 'bg-red-500 text-white shadow-lg shadow-red-500/20'}`}>
                    ...
                    <div className="w-full lg:w-[400px] xl:w-[450px] bg-[#0A0D11] rounded-2xl shadow-2xl border border-white/5 flex flex-col overflow-hidden shrink-0 ring-1 ring-white/5">
                        <div className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-[#0F1216]">
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full ${state === 'connected' ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : state === 'searching' ? 'bg-indigo-500 animate-pulse' : 'bg-gray-500'}`} />
                                <span className="text-xs font-bold text-white uppercase tracking-widest">
                                    {state === 'connected' ? 'Connected' : state === 'searching' ? 'Scanning...' : 'Idle'}
                                </span>
                            </div>
                            <div className="flex items-center gap-1.5 text-white/40">
                               <Globe size={14} />
                               <span className="text-[10px] font-bold uppercase tracking-widest">Global Relay</span>
                            </div>
                        </div>
                        ...
                        <ScrollArea className="flex-1 bg-[#0A0D11] p-4">
                            <div className="space-y-4">
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-white/20 text-xs font-bold uppercase tracking-widest">Encrypted Channel Ready</p>
                                </div>
                                    ...
                                    if (isSystem) {
                                        return (
                                            <div key={i} className="flex justify-center my-4">
                                                <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 rounded-full">{msg.content}</span>
                                            </div>
                                        )
                                    }
                                    return (
                                        <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                            <div className={`max-w-[85%] px-4 py-3 text-sm font-medium ${isMe ? 'bg-indigo-600 text-white rounded-2xl rounded-tr-sm shadow-lg shadow-indigo-500/10' : 'bg-[#1A1D21] text-gray-200 border border-white/5 rounded-2xl rounded-tl-sm'}`}>
                                                {msg.content}
                                            </div>
                                        </div>
                                    )
                        ...
                        <div className="p-4 bg-[#0F1216] border-t border-white/5 space-y-4">
                            <div className="flex gap-3">
                                 {state === 'idle' ? (
                                     <NeuralInformer title="Initialize Search" description="Begin the neural scanning process. This will broadcast your signal to the global matching pool based on your selected interests.">
                                        <button 
                                            id="start-match-btn"
                                            onClick={handleStart}
                                    ...
                                    <NeuralInformer title="Terminate Search" description="Disconnect from the matching queue and cease active signal broadcasting. This will halt all ongoing search protocols.">
                                        <button 
                                            id="stop-match-btn"
                                            onClick={handleStop}
                                ...
                                <NeuralInformer title="Skip/Cycle Protocol" description="Immediately disconnect from the current signal and re-enter the queue. A new connection will be established within seconds.">
                                    <button 
                                        id="next-match-btn"
                                        onClick={handleSkip}
                             ...
                             <div className="flex gap-3">
                                <NeuralInformer title="Abort Link" description="Fully disconnect from the Cospira.LIVE mesh. This will disable all media hardware and revert you to the command dashboard.">
                                    <button 
                                        id="leave-random-btn"
                                        onClick={handleLeave}
                                 ...
                                 <div className="h-11 px-4 bg-[#1A1D21] border border-white/5 rounded-xl flex items-center justify-center min-w-[120px] cursor-pointer hover:bg-[#25282D] transition-colors">
                                     <div className="flex items-center gap-2 text-white/60">
                                         <span className="text-[10px] font-bold uppercase tracking-wider">All Genders</span>
                                     </div>
                                 </div>
                             </div>
