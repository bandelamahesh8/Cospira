...
const PRESET_SEEDS = [
  'Felix', 'Aneka', 'Zack', 'Midnight', 'Bear', 'Tech', 'Cospira', 
  'Luna', 'Leo', 'Max', 'Bella', 'Charlie', 'Lucy', 'Cooper', 'Bailey',
  'Rocky', 'Daisy', 'Milo', 'Sadie', 'Buddy', 'Lola', 'Jack', 'Molly'
];
...
const Profile = () => {
    ...
    return (
      <div className='min-h-full bg-[#05070a] flex items-center justify-center p-6'>
        <div className="relative z-10 max-w-md w-full p-10 bg-[#0c1016] border border-red-500/20 rounded-[40px] text-center shadow-2xl">
            ...
            <h2 className="text-3xl font-black text-white uppercase italic tracking-tighter mb-4">Signal Locked</h2>
            <p className="text-white/40 mb-10 font-medium leading-relaxed uppercase tracking-tight text-xs">Please disconnect from your active session to modify neural identity settings.</p>
  ...
  const handleVerifyPassword = async () => {
      ...
      if (verificationAttempts <= 1) {
          playError();
          toast.error('Access Blocked', { description: 'Too many failed attempts. Identity lock initiated.' });
          setVerificationAttempts(0);
          return;
      }

      setIsLoading(true);
      // We use changePassword with the same password to verify it? 
      // Actually changePassword in useAuth might be a wrapper around supabase.auth.updateUser.
      // Supabase doesn't have a direct "verify current password" beyond re-logging or checking if an update works.
      // However, usually we can try to re-authenticate or use a specific backend endpoint.
      // If I don't have a direct verify endpoint, I might have to mock it or check useAuth.
      
      const { success } = await changePassword(currentPassword, currentPassword);
      setIsLoading(false);
      ...
      if (success) {
          ...
          toast.error('Verification Failed', { description: `Incorrect passkey. ${newAttempts} attempts remaining.` });
      }
  };

  const userEmail = user?.email || 'guest@example.com';
  const currentDisplayName = displayName || user?.user_metadata?.display_name || userEmail.split('@')[0];
  const displayPhotoUrl = previewUrl || photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${currentDisplayName}`;
            ...
            >
                <div className="space-y-4">
                   <div className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-500/10 border border-indigo-500/20 rounded-full">
                       ...
                       <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-200">Identity Protocol</span>
                   </div>
                ...
                <div className="flex items-center gap-4 bg-white/[0.03] border border-white/5 p-4 rounded-3xl backdrop-blur-xl">
                    <div className="flex flex-col items-end">
                        <span className="text-[9px] font-black uppercase tracking-widest text-white/20 leading-none mb-1">Signal Status</span>
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                            <span className="text-sm font-black text-white italic uppercase tracking-tighter">{isConnected ? 'Synchronized' : 'Standalone'}</span>
                        </div>
                    </div>
                </div>
            </motion.div>
                ...
                >
                    <div className="relative group rounded-[40px] bg-[#0c1016]/80 backdrop-blur-xl border border-white/5 shadow-2xl p-10 flex flex-col items-center text-center overflow-hidden">
                        ...
                        <div className="relative mb-8">
                            ...
                            <button 
                                onClick={() => { playClick(); setActiveTab(activeTab === 'avatar' ? 'details' : 'avatar'); }}
                            ...
                            >
                                {activeTab === 'avatar' ? <X className="w-6 h-6" /> : <Camera className="w-6 h-6" />}
                            </button>
                        </div>

                        <h3 className="text-3xl font-black text-white uppercase italic tracking-tighter mb-1 truncate w-full">{currentDisplayName}</h3>
                        <p className="text-[10px] text-white/30 font-mono uppercase tracking-[0.2em] mb-10">CORE_ID: {user?.id?.substring(0,8) || '---'}</p>
                        ...
                        <div className="w-full space-y-3">
                             <div className="p-5 rounded-[2rem] bg-indigo-500/5 border border-indigo-500/10 flex items-center justify-between group/stat">
                                 <div className="flex items-center gap-4">
                                     ...
                                     <span className="text-xs font-black text-white/40 uppercase tracking-widest">Protocol</span>
                                 </div>
                                 <span className="text-xs font-black text-white italic uppercase tracking-tighter">Elite Alpha</span>
                             </div>
                             <div className="p-5 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10 flex items-center justify-between group/stat">
                                 <div className="flex items-center gap-4">
                                     ...
                                     <span className="text-xs font-black text-white/40 uppercase tracking-widest">Trust Index</span>
                                 </div>
                                 <span className="text-xs font-black text-white italic uppercase tracking-tighter">Verified</span>
                             </div>
                        </div>
                    </div>
                </motion.div>
                    ...
                    <div className="bg-[#0c1016]/80 backdrop-blur-xl border border-white/5 rounded-[40px] p-8 md:p-12 relative overflow-hidden">
                        ...
                        <div className="flex gap-8 mb-12 border-b border-white/5">
                            {[
                                ...
                                <button 
                                    key={tab.id}
                                    onClick={() => { playClick(); setActiveTab(tab.id as 'details' | 'avatar' | 'security'); }} 
                                ...
                                >
                                    ...
                                    {tab.label}
                                    {activeTab === tab.id && <motion.div layoutId="profileTab" className="absolute bottom-0 left-0 w-full h-1 bg-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.5)]" />}
                                </button>
                            ))}
                        </div>
                                ...
                                >
                                    <div className="grid gap-8 max-w-2xl">
                                        <div className="space-y-3">
                                            <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400/60 ml-1">DESIGNATION SIGNATURE</Label>
                                        ...
                                        <div className="space-y-3">
                                            <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">ENCRYPTED CHANNEL</Label>
                                            <div className="relative block">
                                                <Mail className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10" />
                                                <Input 
                                                    value={userEmail}
                                                    disabled
                                                    className="h-16 pl-16 bg-white/[0.02] border-white/5 rounded-[20px] font-mono text-sm text-white/30 cursor-not-allowed italic"
                                                />
                                                <div className="absolute right-6 top-1/2 -translate-y-1/2 flex items-center gap-2 px-3 py-1.5 bg-emerald-500/5 rounded-xl border border-emerald-500/20">
                                        ...
                                        <div className="space-y-3">
                                            <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">NEURAL FREQUENCY</Label>
                                            <Select value={gender} onValueChange={setGender}>
                                                ...
                                                <SelectContent className="bg-[#0c1016] border-white/5 text-white rounded-2xl overflow-hidden">
                                                    <SelectItem value="male" className="h-12 uppercase italic font-bold">Male Signal</SelectItem>
                                                    <SelectItem value="female" className="h-12 uppercase italic font-bold">Female Signal</SelectItem>
                                                    <SelectItem value="other" className="h-12 uppercase italic font-bold">Neutral Matrix</SelectItem>
                                                </SelectContent>
                                            </Select>
                                        </div>
                                    </div>
                                ...
                                >
                                     ...
                                     >
                                         ...
                                         <div className="text-center">
                                             <h3 className="text-xl font-black text-white italic uppercase tracking-tighter">Inject Visual Data</h3>
                                             <p className="text-[10px] text-white/20 mt-1 font-black uppercase tracking-[0.2em]">MAX FILESIZE: 5MB • PNG / JPG / SVG</p>
                                         </div>
                                              ...
                                              <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-4">
                                                  {PRESET_SEEDS.map((seed) => {
                                                      ...
                                                      return (
                                                          <motion.button 
                                                             key={seed}
                                                             onClick={() => { playClick(); handleSelectPreset(seed); }}
                                                          ...
                                                          >
                                                              <img src={seedUrl} alt={seed} className="w-full h-full object-cover bg-[#05070a]" />
                                                              {isActive && (
                            ...
                            {activeTab === 'security' && (
                                ...
                                >
                                      <div className="grid gap-8">
                                          <div className="space-y-3">
                                              <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-white/20 ml-1">IDENTITY VERIFICATION</Label>
                                              <div className="relative group">
                                                  <KeyRound className={`absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${isVerified ? 'text-emerald-400' : 'text-white/10 group-focus-within:text-indigo-400'}`} />
                                                  <Input 
                                                      type="password"
                                                  ...
                                                  {!isVerified && verificationAttempts > 0 && (
                                                      <button 
                                                        onClick={(e) => { e.preventDefault(); playClick(); handleVerifyPassword(); }}
                                              ...
                                              {isVerified && (
                                                  ...
                                                  >
                                                      <div className="grid md:grid-cols-2 gap-6 pt-4">
                                                          <div className="space-y-3">
                                                              <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500/60 ml-1">NEW CREDENTIAL</Label>
                                                              <div className="relative group">
                                                          ...
                                                          <div className="space-y-3">
                                                              <Label className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500/60 ml-1">RE-VERIFY CREDENTIAL</Label>
                                                              <div className="relative group">
                                                                  <Lock className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/10 group-focus-within:text-emerald-400 transition-colors" />
                                                                  <Input 
                                                                      type="password"
                                                      ...
                                                      <motion.button 
                                                          onClick={() => { playClick(); handleChangePassword(); }}
                                                          disabled={isLoading}
                                          ...
                                          {!isVerified && verificationAttempts < 3 && (
                                              <p className="text-[10px] font-black uppercase tracking-widest text-red-400 italic">
                                                  Verification failed. {verificationAttempts} attempts remaining before identity lock.
                                              </p>
                                          )}
                                      </div>
                                     ...
                                     <div className="pt-10 flex flex-col gap-6 border-t border-white/5">
                                         ...
                                         <div className="flex items-center justify-between px-4 py-6 bg-white/[0.02] border border-white/5 rounded-3xl">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] text-white/30 font-black uppercase tracking-widest">Forgotten terminal passkey?</span>
                                            </div>
                                            <button 
                ...
                >
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 pb-6 border-b border-white/5">
                        <div className="flex items-center gap-4">
                            ...
                            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter">Engagement <span className="text-white/20">Logs</span></h2>
                        </div>
                        <div className="flex items-center gap-4 bg-white/5 px-4 py-2 rounded-2xl border border-white/5">
                            <span className="text-[10px] font-black uppercase tracking-widest text-white/20">Archive Status</span>
                            <span className="text-xs font-black text-emerald-400 uppercase tracking-tighter italic">Secured</span>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {!recentRooms || recentRooms.length === 0 ? (
                            <div className="md:col-span-2 py-24 border border-dashed border-white/5 rounded-[32px] flex flex-col items-center justify-center bg-white/[0.01]">
                                ...
                                <span className="text-xs font-black uppercase tracking-[0.3em] text-white/20">No active signal history recorded</span>
                                <button 
                            ...
                            recentRooms.slice(0, 10).map((room: { id: string, name?: string, userCount?: number }, i) => (
                                <motion.div 
                                    key={room.id}
                                    initial={{ opacity: 0, x: -10 }}
                                    animate={{ opacity: 1, x: 0 }}
                                    transition={{ delay: 0.4 + (i * 0.05) }}
                                    onClick={() => { playClick(); navigate(`/room/${room.id}`); }}
                                ...
                                >
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <h4 className="text-xl font-black text-white group-hover:text-indigo-400 transition-colors uppercase italic tracking-tighter">{room.name || 'SECURE SECTOR'}</h4>
                                            <div className="flex items-center gap-3 mt-1 opacity-40">
                                                <span className="text-[10px] font-mono font-bold uppercase tracking-widest">ID: {room.id}</span>
                                                <div className="w-1 h-1 rounded-full bg-white/30" />
                                                <span className="text-[10px] font-black uppercase tracking-widest">{room.userCount || 0} ACTIVE</span>
                                            </div>
                                        </div>
                                    </div>
