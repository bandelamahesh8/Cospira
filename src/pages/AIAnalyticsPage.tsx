...
import { 
  Clock, Shield, Zap, Users, Target, Activity, 
  Layers, RefreshCw, 
  Search, Globe, Lock, Cpu, Check
} from 'lucide-react';
...
const PurgeConfirmModal = ({ isOpen, onClose, onConfirm, loading }: { isOpen: boolean; onClose: () => void; onConfirm: () => void; loading: boolean }) => {
    ...
    <div className='fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm'>
       ...
       >
         ...
         <div className='flex items-center gap-4 mb-6'>
           ...
           <div>
             <h3 className='text-2xl font-black italic uppercase tracking-tighter text-white'>Neural Purge</h3>
             <p className='text-[10px] font-bold text-red-500/50 uppercase tracking-widest mt-1'>Irreversible Operation</p>
           </div>
         </div>
         <p className='text-sm text-white/60 leading-relaxed uppercase font-bold tracking-tight mb-8 italic'>
           Are you sure you want to clear your neural history? This will reset all your analytics, activity feeds, and sector connectivity logs.
         </p>
...
export const AIAnalyticsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'activity' | 'sectors' | 'signals'>('overview');
  ...
  const fetchData = useCallback((silent = false) => {
    ...
    // Fetch AI insights
    socket.emit('get-user-ai-insights', { userId: user?.id }, (res: { success: boolean; insights?: AIInsights }) => {
      if (res.success && res.insights) setInsights(res.insights);
      checkDone();
    });

    // Fetch User Activity
    socket.emit('get-user-activity', { limit: 100, userId: user?.id }, (res: { success: boolean; activities?: ActivityItem[] }) => {
          ...
          >
            <div>
              ...
              <div className='flex flex-wrap items-center gap-4 text-white/40 font-bold uppercase tracking-tight text-sm'>
                <span>Analyzing signature: <span className='text-white italic'>{user?.user_metadata?.display_name || user?.email?.split('@')[0]}</span></span>
                <div className='w-1.5 h-1.5 rounded-full bg-cyan-500/30' />
                <span>Sector Integrity: <span className='text-emerald-400'>{(insights?.securityCompliance || 100).toFixed(1)}%</span></span>
              </div>
            </div>
            ...
            <div className='flex flex-col gap-4'>
              <div className='flex items-center gap-4 px-6 py-4 bg-white/[0.03] border border-white/5 rounded-3xl backdrop-blur-2xl shadow-2xl'>
                <div className={`w-2.5 h-2.5 rounded-full ${isConnected ? 'bg-cyan-500 animate-pulse' : 'bg-red-500'} shadow-[0_0_10px_rgba(6,182,212,0.5)]`} />
                <span className='text-[10px] font-black uppercase tracking-widest text-white/60'>
                  {isConnected ? 'Neural Link Active' : 'Uplink Failed'}
                </span>
                <div className='w-px h-8 bg-white/10 mx-2' />
                <button onClick={handleClearHistory} className='flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-red-500 hover:text-red-400 transition-colors group'>
                    ...
                    <div className='flex items-center justify-between mb-12 relative z-10'>
                      <div className='flex items-center gap-4'>
                        ...
                        <div>
                          ...
                          <p className='text-[10px] font-bold text-white/30 uppercase tracking-widest mt-1'>Signal density over 7 cycles</p>
                        </div>
                      </div>
                      <div className='flex gap-3'>
                        {[
                          ...
                          <div key={m.label} className='px-5 py-2.5 bg-white/5 border border-white/10 rounded-2xl'>
                  ...
                  <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
                    <div className='bg-[#0c1016]/40 backdrop-blur-3xl border border-white/5 rounded-[40px] p-8 overflow-hidden relative group shadow-xl'>
                       <div className='flex items-center gap-4 mb-6 relative z-10'>
                          ...
                          <h3 className='text-xl font-black uppercase tracking-tighter italic'>Web Sector Intelligence</h3>
                       </div>
                       <p className='text-sm text-white/40 uppercase font-bold tracking-tight mb-8 relative z-10'>Most connected clusters within the Cospira network.</p>
                        <div className='space-y-4 relative z-10'>
                          {(insights?.topOrganizations || []).map((org, i) => (
                            <div key={i} className='flex items-center justify-between p-4 bg-white/[0.02] border border-white/5 rounded-2xl hover:bg-white/5 transition-colors group/item cursor-pointer'>
                               <div className='flex items-center gap-4'>
                                 ...
                                 <span className='font-black uppercase italic text-sm text-white/80 group-hover/item:text-cyan-400 transition-colors'>{org.name}</span>
                               </div>
                               <span className='text-[10px] font-mono font-bold text-white/20 uppercase'>{org.count} Logins</span>
                            </div>
                          ))}
                          {(!insights?.topOrganizations || insights.topOrganizations.length === 0) && (
                            <div className='text-center py-4 text-[10px] font-black uppercase text-white/20 italic'>No network clusters detected</div>
                          )}
                       </div>
                    </div>
                    ...
                    <div className='bg-[#0c1016]/40 backdrop-blur-3xl border border-white/5 rounded-[40px] p-8 overflow-hidden relative group shadow-xl'>
                       <div className='flex items-center gap-4 mb-6 relative z-10'>
                          ...
                          <h3 className='text-xl font-black uppercase tracking-tighter italic'>Security Vector</h3>
                       </div>
                       <div className='space-y-6 relative z-10'>
                          <div className='flex items-end justify-between'>
                            <div>
                               <p className='text-sm font-black italic uppercase text-white/40'>Encryption Health</p>
                               <p className='text-4xl font-black text-white italic tracking-tighter uppercase'>Stable</p>
                            </div>
                            <p className='text-4xl font-black text-emerald-400 italic tracking-tighter'>{(insights?.securityCompliance || 100).toFixed(1)}%</p>
                          </div>
                          ...
                          <div className='p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl'>
                             <p className='text-[9px] font-black uppercase text-emerald-400/60 leading-relaxed tracking-widest italic'>Neural guard active. No significant breaches detected in recent cycles.</p>
                          </div>
                       </div>
                    </div>
                  </div>
                ...
                <div className='lg:col-span-4 space-y-8'>
                  <div className='bg-gradient-to-b from-[#0c1016] to-transparent backdrop-blur-3xl border border-white/10 rounded-[48px] p-10 shadow-2xl flex flex-col h-full'>
                    ...
                    <div className='flex-1 space-y-12'>
                      {/* Breakdown Bars */}
                      {[
                        { label: 'Organization Units', icon: Globe, value: insights?.sectorBreakdown?.organization || 0, color: '#a855f7' },
                        { label: 'Private Clusters', icon: Lock, value: insights?.sectorBreakdown?.private || 0, color: '#06b6d4' }
                      ].map((item, i) => (
                        <div key={i} className='space-y-4'>
                          <div className='flex justify-between items-end'>
                            <span className='text-[10px] font-black uppercase tracking-[0.3em] text-white/30'>{item.label}</span>
                            <span className='text-2xl font-black italic text-white'>{item.value}</span>
                          </div>
                          <div className='h-4 bg-white/5 rounded-full p-1 overflow-hidden'>
                            <motion.div 
                              initial={{ width: 0 }}
                              animate={{ width: `${item.value > 0 ? Math.max(2, (item.value / ((insights?.roomsJoined || 1) + 1)) * 100) : 0}%` }}
                      ...
                      <div className='mt-12 p-8 rounded-[40px] bg-[#14b8a6]/5 border border-[#14b8a6]/20 relative overflow-hidden group hover:border-[#14b8a6]/40 transition-colors'>
                        ...
                        <div className='flex items-center gap-3 mb-4'>
                          ...
                          <span className='text-[10px] font-black uppercase tracking-widest text-[#14b8a6]'>Intelligence Rank</span>
                        </div>
                        <h4 className='text-3xl font-black text-white italic uppercase tracking-tighter mb-2'>{insights?.rank}</h4>
                        <p className='text-xs text-white/40 uppercase font-bold tracking-tight italic'>Validated across {insights?.roomsCreated} creation events and {insights?.roomsJoined} network joins.</p>
                      </div>
                      ...
                      <div className='pt-8 border-t border-white/5 space-y-4'>
                        <div className='flex items-center justify-between text-[11px] font-black uppercase text-white/30 tracking-widest'>
                          ...
                          <span className='text-white italic'>{insights?.averageSessionDuration}m</span>
                        </div>
                        <div className='flex items-center justify-between text-[11px] font-black uppercase text-white/30 tracking-widest'>
                          <span>Total Network Pulse</span>
                          <span className='text-white italic'>{insights?.totalMessages} Events</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            >
              <div className='grid grid-cols-1 lg:grid-cols-2 gap-12'>
                 <div className='bg-[#0c1016]/60 backdrop-blur-3xl border border-white/10 rounded-[48px] p-12 shadow-2xl'>
                    <h3 className='text-3xl font-black italic uppercase tracking-tighter mb-8'>Sector <span className='text-cyan-400'>Dominance</span></h3>
                    <div className='h-[400px] w-full'>
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={[
                          { name: 'Organization', value: insights?.sectorBreakdown?.organization || 0 },
                             ...
                             if (active && payload?.[0]) {
                               return (
                                 <div className='bg-[#0c1016]/90 border border-white/10 p-4 rounded-3xl backdrop-blur-xl'>
                                    ...
                                    <p className='text-2xl font-black text-cyan-400 italic'>{payload[0].value} Rooms</p>
                                 </div>
                               )
                             }
                             return null;
                          }} />
                           <Bar dataKey="value" radius={[20, 20, 20, 20]} barSize={80} minPointSize={10}>
                            {
                              [
                                { name: 'Organization', value: insights?.sectorBreakdown?.organization || 0 },
                                { name: 'Private', value: insights?.sectorBreakdown?.private || 0 }
                    ...
                    <div className='flex justify-center gap-12 mt-8 font-black uppercase italic tracking-tighter'>
                       <div className='flex items-center gap-3'><div className='w-3 h-3 rounded-full bg-purple-500'/> <span>Organization</span></div>
                       <div className='flex items-center gap-3'><div className='w-3 h-3 rounded-full bg-cyan-500'/> <span>Private</span></div>
                    </div>
                 </div>
                 ...
                 <div className='space-y-8'>
                    <div className='bg-[#0c1016]/40 border border-white/5 rounded-[40px] p-10 flex flex-col justify-center'>
                       <div className='flex items-center gap-4 mb-8'>
                          ...
                          <h3 className='text-2xl font-black uppercase tracking-tighter italic'>Creation Analytics</h3>
                       </div>
                       <div className='grid grid-cols-2 gap-6'>
                          <div className='bg-white/5 p-6 rounded-3xl border border-white/5'>
                             <p className='text-[10px] font-black text-white/20 uppercase tracking-widest mb-2'>Rooms Created</p>
                             <p className='text-4xl font-black italic'>{insights?.roomsCreated || 0}</p>
                          </div>
                          <div className='bg-white/5 p-6 rounded-3xl border border-white/5'>
                             <p className='text-[10px] font-black text-white/20 uppercase tracking-widest mb-2'>Creation Rank</p>
                             <p className='text-2xl font-black italic uppercase text-cyan-400'>{insights?.rank.split(' ')[0]}</p>
                          </div>
                       </div>
                    </div>

                    <div className='bg-[#0c1016]/40 border border-white/5 rounded-[40px] p-10'>
                       <h4 className='text-sm font-black uppercase text-white/30 tracking-[0.2em] mb-8 italic'>Sector Distribution Summary</h4>
                       <div className='space-y-8'>
                          <p className='text-lg font-bold text-white/60 leading-relaxed uppercase italic tracking-tight font-black'>
                            {((insights?.sectorBreakdown?.organization || 0) + (insights?.sectorBreakdown?.private || 0)) > 0 ? (
                              <>
                                Your signature predominantly operates within <span className='text-cyan-400'>{(insights?.sectorBreakdown?.organization || 0) > (insights?.sectorBreakdown?.private || 0) ? 'ORGANIZATION' : 'PRIVATE'}</span> clusters, 
                                which account for <span className='text-white'>{(Math.max(insights?.sectorBreakdown?.organization || 0, insights?.sectorBreakdown?.private || 0) / ((insights?.sectorBreakdown?.organization || 0) + (insights?.sectorBreakdown?.private || 0)) * 100).toFixed(1)}%</span> of your total signal footprint.
                              </>
                            ) : (
                          ...
                          <div className='p-6 bg-white/5 border border-white/10 rounded-3xl flex items-center gap-6 relative overflow-hidden group/heur'>
                             ...
                             <div className={`w-14 h-14 rounded-full border-4 flex items-center justify-center transition-all duration-1000 ${
                               loading ? 'border-cyan-500 border-t-transparent animate-spin' : 'border-cyan-500/50 bg-cyan-500/10 shadow-[0_0_20px_rgba(6,182,212,0.2)]'
                             }`}>
                             ...
                             <div className='relative z-10'>
                                <p className='text-xs font-black text-white italic uppercase tracking-wider'>
                                  {loading ? 'Analyzing Network Tendencies...' : 'Heuristic Analysis Complete'}
                                </p>
                                <p className='text-[10px] font-bold text-white/30 uppercase tracking-widest mt-1'>
                                  {loading ? 'Mapping neural sector signatures...' : 'Heuristic analysis suggests a high collaborative affinity.'}
                                </p>
                             </div>
                          </div>
                       </div>
            ...
            >
               <div className='bg-[#0c1016]/60 backdrop-blur-3xl border border-white/10 rounded-[48px] p-6 md:p-12 shadow-2xl'>
                 <div className='flex items-center gap-4 mb-12'>
                    ...
                    <div>
                      <h3 className='text-3xl font-black italic uppercase tracking-tighter text-white'>Neural <span className='text-cyan-400'>Timeline</span></h3>
                      <p className='text-xs font-bold text-white/30 uppercase tracking-widest mt-1'>Chronological flow of your actions</p>
                    </div>
                 </div>

                 {loading ? (
                    <div className='flex justify-center p-20'><RefreshCw className='animate-spin text-cyan-500 size-12 opacity-50' /></div>
                 ) : activities.length > 0 ? (
                      ...
                      {activities.map((activity, index) => (
                        ...
                        >
                          <div className='flex items-center justify-center w-14 h-14 rounded-full border border-white/10 bg-[#0c1016] text-white/40 shadow-2xl shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 group-hover:bg-cyan-500/10 group-hover:text-cyan-400 group-hover:border-cyan-500/30 transition-all z-10'>
                             {activity.type === 'room' ? <Globe size={18} /> : activity.type === 'social' ? <Users size={18} /> : <Zap size={18} />}
                          </div>
                          ...
                          <div className='w-[calc(100%-4.5rem)] md:w-[calc(50%-3rem)] p-6 rounded-[2rem] bg-white/[0.02] border border-white/5 group-hover:border-cyan-500/20 transition-all shadow-xl backdrop-blur-sm'>
                            ...
                            <p className='text-sm font-black text-white/70 italic uppercase tracking-tight'>{activity.subtitle}</p>
                            {activity.duration && (
                               <div className='mt-4 pt-4 border-t border-white/5'>
                                  <p className='text-[10px] font-mono text-white/30'>DURATION: <span className='text-cyan-400/80 font-bold'>{(activity.duration/60).toFixed(1)} MIN</span></p>
                               </div>
                            )}
                          </div>
                        </motion.div>
                    ...
                    <div className='text-center py-20 border border-dashed border-white/5 rounded-[40px]'>
                       <p className='text-xl text-white/20 font-black uppercase italic tracking-[0.2em]'>No neural activity detected.</p>
                       <p className='text-xs text-white/10 uppercase tracking-widest mt-4'>Your footprint is currently clean.</p>
                    </div>
                 )}
               </div>
            </motion.div>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
                {loading ? (
                  ...
                  filteredHistory.map((item, i) => (
                    ...
                    >
                       <div className='absolute top-0 right-0 p-6'>
                          <div className={`w-3 h-3 rounded-full ${item.isActive ? 'bg-emerald-500 shadow-[0_0_10px_#10b981]' : 'bg-white/10'} transition-all`} />
                       </div>
                       
                       <div className='relative z-10'>
                          <div className='flex items-center gap-4 mb-4'>
                             <div className={`p-3 rounded-xl ${item.type === 'organization' ? 'bg-purple-500/10 text-purple-400' : 'bg-cyan-500/10 text-cyan-400'}`}>
                                {item.type === 'organization' ? <Globe size={20} /> : <Lock size={20} />}
                             </div>
                             <span className='text-[10px] font-black uppercase tracking-widest text-white/30'>{item.type}</span>
                          </div>
                          <h3 className='text-2xl font-black italic uppercase tracking-tighter text-white group-hover:text-cyan-400 transition-colors truncate max-w-[80%]'>{item.name}</h3>
                       </div>
                       ...
                       <div className='flex items-center justify-between pt-6 border-t border-white/5 mt-6'>
                          <div className='flex items-center gap-2'>
                             ...
                             <span className='text-[10px] font-black uppercase text-white/40 tracking-widest'>{item.participantCount || 0} Agents</span>
                          </div>
                       </div>
                    </motion.div>
                  ))
                  ...
                  <div className='col-span-full h-96 flex flex-col items-center justify-center bg-white/[0.01] border border-dashed border-white/10 rounded-[60px] opacity-40'>
                    ...
                    <p className='text-xl font-black uppercase italic tracking-[0.3em]'>System Standby: Zero Signals Found</p>
                  </div>
                )}
              </div>
            </motion.div>
