...
const Dashboard = () => {
    ...
    const handleCreate = async () => {
        ...
        if ((mode === 'pro' || mode === 'ultra') && !password.trim()) {
            toast.error("Security Protocol Alert", { description: `${mode.toUpperCase()} mode requires a password.` });
            return;
        }
        ...
        <AnimatePresence>
            {isAiActive && (
                ...
                >
                    ...
                    <div className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-emerald-500/[0.03] rounded-full blur-[140px] animate-pulse" style={{ animationDelay: '1.5s' }} />
                </motion.div>
            )}
        </AnimatePresence>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pt-8 px-2 pb-2 relative z-10">
            ...
            <div className="flex flex-col md:flex-row items-start justify-between gap-8 mb-8">
                <div>
                     <div className="flex items-center gap-4 mb-2">
                        ...
                        <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">Command Center</span>
                     </div>
                     <h2 className="text-5xl font-black text-white italic uppercase tracking-tighter mb-2">DASHBOARD</h2>
                     <p className="text-zinc-500 font-medium">Welcome, <span className="text-white">Architect</span>. System running smoothly.</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 items-start relative z-20">
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex-1">
                            <span className="text-lg font-black text-black uppercase tracking-tight leading-none block">CREATE ROOM</span>
                            <span className="text-[10px] font-bold text-black/40 uppercase tracking-widest block">Sector Name</span>
                        </div>
                        <ArrowRight className={`w-5 h-5 text-black/30 transition-transform duration-300 ${isCreateOpen ? 'rotate-90' : 'group-hover:translate-x-1'}`} />
                    </div>
                </NeuralInformer>
                ...
                >
                    ...
                    >
                        <div className="w-10 h-10 rounded-xl bg-[#1A1D24] border border-white/5 flex items-center justify-center mr-4 relative z-10 text-blue-400">
                            <LogIn className={`w-5 h-5 transition-transform duration-500 ${isJoinOpen ? 'scale-110' : ''}`} />
                        </div>
                        <div className="flex-1 relative z-10">  
                            <span className="text-lg font-black text-white uppercase tracking-tight leading-none block">JOIN ROOM</span>
                            <span className="text-[10px] font-bold text-blue-500/50 uppercase tracking-widest block">Access Uplink</span>
                        </div>
                        <ArrowRight className={`w-5 h-5 text-white/10 relative z-10 transition-transform duration-300 ${isJoinOpen ? 'rotate-90' : 'group-hover:translate-x-1'}`} />
                    </div>
                </NeuralInformer>
                ...
                >
                    ...
                    >
                        <div className={`absolute inset-0 rounded-[24px] bg-gradient-to-r from-indigo-900/20 to-purple-900/20 transition-opacity duration-300 ${isGlobalOpen ? 'opacity-100' : 'opacity-50'}`} />
                        <div className="w-10 h-10 rounded-xl bg-[#1A1A24] border border-white/5 flex items-center justify-center mr-4 relative z-10">
                            <Shuffle className={`w-5 h-5 text-purple-400 transition-transform duration-500 ${isGlobalOpen ? 'rotate-180' : ''}`} />
                        </div>
                        <div className="flex-1 relative z-10">  
                            <span className="text-lg font-black text-white uppercase tracking-tight leading-none block">Global Match</span>
                            <span className="text-[10px] font-bold text-purple-500/50 uppercase tracking-widest block">Connect Across Global</span>
                        </div>
                        <ArrowRight className={`w-5 h-5 text-white/10 relative z-10 transition-transform duration-300 ${isGlobalOpen ? 'rotate-90' : 'group-hover:translate-x-1'}`} />
                    </div>
                </NeuralInformer>
            </div>

            {/* Expandable Content Area - Full Width */}
            <div className={`grid transition-all duration-300 ease-in-out ${isCreateOpen || isJoinOpen || isGlobalOpen ? 'grid-rows-[1fr] mb-8' : 'grid-rows-[0fr] mb-0'}`}>
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    {/* Room Name Input */}
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Room Name</label>
                                    ...
                                    {/* Password Input */}
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Password</label>
                                        <div className="bg-[#0F1116] border border-white/5 focus-within:border-white/20 transition-colors rounded-2xl h-[60px] flex items-center px-6">
                                            <input 
                                                type="password"
                                                value={password}
                                                onChange={(e) => setPassword(e.target.value)}
                                                placeholder={(mode === 'pro' || mode === 'ultra') ? "PASSWORD (REQUIRED)" : "PASSWORD (OPTIONAL)"}
                                                className="bg-transparent border-none outline-none text-sm font-bold text-white uppercase tracking-widest p-0 h-auto placeholder:text-zinc-700 w-full focus:ring-0"
                                            />
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    {/* Privacy Toggle */}
                                    <div>
                                         <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Privacy Access</label>
                                         <div className="grid grid-cols-2 gap-4">
                                            ...
                                            >
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${privacy === 'public' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-white/5 text-zinc-500'}`}>
                                                    <Globe className="w-4 h-4" />
                                                </div>
                                                <div className="text-left leading-none">
                                                    <div className={`text-[10px] font-black uppercase ${privacy === 'public' ? 'text-white' : 'text-zinc-500'}`}>Public Sector</div>
                                                    <div className="text-[8px] font-bold text-emerald-500/50 uppercase tracking-wider">Open Uplink</div>
                                                </div>
                                            </button>
                                            <button 
                                            ...
                                            >
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${privacy === 'encrypted' ? 'bg-indigo-500/20 text-indigo-500' : 'bg-white/5 text-zinc-500'}`}>
                                                    <Shield className="w-4 h-4" />
                                                </div>
                                                <div className="text-left leading-none">
                                                    <div className={`text-[10px] font-black uppercase ${privacy === 'encrypted' ? 'text-white' : 'text-zinc-500'}`}>Encrypted</div>
                                                    <div className="text-[8px] font-bold text-indigo-500/50 uppercase tracking-wider">Passkey Locked</div>
                                                </div>
                                            </button>
                                         </div>
                                    </div>
                                     ...
                                     {/* Protocol Toggle */}
                                     <div>
                                         <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Sector Protocol</label>
                                         <div className="grid grid-cols-4 gap-4">
                                            {['fun', 'pro', 'ultra', 'mixed'].map((m) => {
                                                ...
                                                if(m === 'fun') {
                                                    ...
                                                    infoDesc = "Optimized for social high-engagement. Open security and casual gaming presets enabled.";
                                                } else if(m === 'pro') {
                                                    infoTitle = "Professional Collaboration";
                                                    infoDesc = "Low-latency screensharing and high-fidelity audio. Forced encryption for secure data exchange.";
                                                } else if(m === 'ultra') {
                                                    infoTitle = "High-Bandwidth Performance";
                                                    infoDesc = "Advanced SFU routing for 4K streams. Sub-50ms latency for critical real-time operations.";
                                                } else if(m === 'mixed') {
                                                    infoTitle = "Versatile Neural State";
                                                    infoDesc = "Automatically balances resources between social interaction and professional tooling.";
                                                }
                                                ...
                                                return (
                                                    <NeuralInformer key={m} title={infoTitle} description={infoDesc}>
                                                        ...
                                                        >
                                                            {m === 'fun' && <Gamepad2 className={`w-3 h-3 ${mode === m ? 'text-purple-500' : 'text-zinc-600'}`} />}
                                                            {m === 'pro' && <Briefcase className={`w-3 h-3 ${mode === m ? 'text-blue-500' : 'text-zinc-600'}`} />}
                                                            {m === 'ultra' && <Lock className={`w-3 h-3 ${mode === m ? 'text-red-500' : 'text-zinc-600'}`} />}
                                                            {m === 'mixed' && <Shuffle className={`w-3 h-3 ${mode === m ? 'text-amber-500' : 'text-zinc-600'}`} />}
                                                            <span className={`text-[9px] font-black uppercase tracking-widest ${mode === m ? 'text-white' : 'text-zinc-600'}`}>{m}</span>
                                                        </button>
                                                    </NeuralInformer>
                                                );
                                            })}
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                            <div className="flex flex-col md:flex-row items-center gap-6">
                                ...
                                <div className="flex-1 w-full space-y-4">
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Entry Protocol</label>
                                    ...
                                    <div className="px-4">
                                        <p className="text-[9px] text-zinc-500 font-medium">Entering a valid signal frequency will bypass public discovery.</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-row md:flex-col items-center gap-3 opacity-30">
                                    ...
                                    <div className="text-[10px] font-black text-white uppercase tracking-tighter">OR</div>
                                    <div className="h-px w-8 md:w-px md:h-10 bg-white" />
                                </div>
                                ...
                                <div className="flex-1 w-full space-y-4">
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Direct Invite Link</label>
                                        <div className="bg-[#0F1116] border border-white/5 focus-within:border-blue-500/30 transition-all rounded-2xl h-[60px] flex items-center px-6 gap-4">
                                    ...
                                    <div className="px-4">
                                         <p className="text-[9px] text-zinc-500 font-medium italic">Auto-extracting sector ID from beams.</p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-top-4 duration-300">
                            ...
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[9px] font-black text-white uppercase tracking-widest">Protocol Configuration</span>
                                </div>
                                <div className="bg-emerald-500/10 border border-emerald-500/20 px-2 py-1 rounded-md flex items-center gap-1.5">
                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                    <span className="text-[8px] font-bold text-emerald-500 uppercase tracking-wider">3 Live Agents</span>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-4">
                                    {/* Interface Priority */}
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Interface Priority</label>
                                        <div className="grid grid-cols-2 gap-4">
                                            <button 
                                            ...
                                            >
                                                <div className={`w-2 h-2 rounded-full ${matchMode === 'video' ? 'bg-indigo-500 shadow-[0_0_6px_#6366f1]' : 'bg-transparent'}`} />
                                            ...
                                            >
                                                <div className={`w-2 h-2 rounded-full ${matchMode === 'chat' ? 'bg-indigo-500 shadow-[0_0_6px_#6366f1]' : 'bg-transparent'}`} />
                                    ...
                                    {/* Common Interests */}
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Common Interests (Optional)</label>
                                        <div className="bg-[#0F1116] border border-white/5 focus-within:border-white/20 transition-colors rounded-2xl h-[60px] flex items-center px-6">
                                            <input 
                                                value={interests}
                                ...
                                <div className="space-y-4">
                                    {/* Connect Intent */}
                                    <div>
                                        <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Connect Intent</label>
                                        <div className="grid grid-cols-2 gap-4">
                                            {[
                                                ...
                                                <button 
                                                    key={item.id}
                                                    onClick={() => setIntent(item.id as 'casual' | 'focus' | 'network' | 'play')}
                                                ...
                                                >
                                                     <div className={`w-8 h-8 rounded-full flex items-center justify-center ${intent === item.id ? 'bg-emerald-500/20' : 'bg-white/5'}`}>
                            ...
                            >
                                <button 
                                    onClick={() => navigate('/connect', { state: { mode: matchMode, intent, interests: interests.split(',').filter(Boolean) } })}
                 ...
                 <div className="lg:col-span-2">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2">
                           ...
                           <h3 className="text-xs font-black text-white uppercase tracking-widest">Live Signals</h3>
                        </div>
                        <div className="flex items-center gap-4">
                           <div className="flex items-center gap-3">
                               <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /><span className="text-[9px] font-bold text-zinc-500 uppercase">Online</span></div>
                               <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-orange-500" /><span className="text-[9px] font-bold text-zinc-500 uppercase">Busy</span></div>
                               <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-red-500" /><span className="text-[9px] font-bold text-zinc-500 uppercase">Idle</span></div>
                           </div>
                           <button 
                                onClick={() => toast.success('Terminal Synced', { description: 'Latest global signals received.' })}
