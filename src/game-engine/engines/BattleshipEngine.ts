...
export class BattleshipEngine extends BaseGameEngine {
  ...
  initGame(players: Player[]): GameState {
    ...
    players.forEach(p => {
        board[p.id] = {
            grid: Array(this.BOARD_SIZE).fill(null).map(() => Array(this.BOARD_SIZE).fill('empty')),
            ships: [],
            isReady: false,
            shotsFired: []
        };
  ...
  validateMove(move: Move, state: GameState): ValidationResult {
    ...
    if (phase === 'BATTLE') {
        ...
        const { r, c } = move.data;
        if (r < 0 || r >= this.BOARD_SIZE || c < 0 || c >= this.BOARD_SIZE) return { valid: false, reason: 'Out of bounds' };
  ...
  applyMove(move: Move, state: GameState): GameState {
    ...
    if (phase === 'SETUP') {
        if (move.type === 'place_ships') {
            ...
            board.grid = Array(this.BOARD_SIZE).fill(null).map(() => Array(this.BOARD_SIZE).fill('empty'));
        ...
        if (targetBoard.grid[r][c] === 'ship') {
            ...
            // Update ship hits
            const ship = targetBoard.ships.find((s: Ship) => s.coords.some(coord => coord.r === r && coord.c === c));
            if (ship) ship.hits++;
