...
export class ConnectFourEngine extends BaseGameEngine {
  ...
  initGame(players: Player[]): GameState {
    ...
    const board: Board = Array(this.ROWS).fill(null).map(() => Array(this.COLS).fill(null));
  ...
  applyMove(move: Move, state: GameState): GameState {
    ...
    const newState: GameState = {
      ...
      metadata: {
        ...
        moveHistory: [...state.metadata.moveHistory, { playerId: move.playerId, col, row: rowIndex }],
      },
      updatedAt: new Date(),
    };
  ...
  private checkDirection(board: Board, r: number, c: number, dr: number, dc: number, color: string): boolean {
