...
export class BilliardsEngine extends BaseGameEngine {
  ...
  private stepPhysics(balls: Ball[]) {
      balls.forEach(ball => {
          ...
          if (ball.x > TABLE_WIDTH - BALL_RADIUS) { ball.x = TABLE_WIDTH - BALL_RADIUS; ball.vx *= -1; }
          if (ball.y < BALL_RADIUS) { ball.y = BALL_RADIUS; ball.vy *= -1; }
          if (ball.y > TABLE_HEIGHT - BALL_RADIUS) { ball.y = TABLE_HEIGHT - BALL_RADIUS; ball.vy *= -1; }
  ...
  private setupRack(): Ball[] {
      ...
      // 1. Cue Ball
      balls.push({ id: 0, x: TABLE_WIDTH * 0.25, y: startY, vx: 0, vy: 0, type: 'white', visible: true });
