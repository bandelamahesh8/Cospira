...
export class CheckersEngine extends BaseGameEngine {
  ...
  checkWinner(state: GameState): WinnerResult {
    ...
    if (redCount === 0)
      return { finished: true, winner: state.players.find((p) => p.role === 'white')?.id || null };
    if (whiteCount === 0)
      return { finished: true, winner: state.players.find((p) => p.role === 'red')?.id || null };
  ...
  getValidMoves(state: GameState): Move[] {
    ...
    if (mustJump && jumpSource) {
      this.getJumpsForPiece(
        board,
        jumpSource.r,
        jumpSource.c,
        board[jumpSource.r][jumpSource.c]!,
        moves,
        state.currentTurn
      );
      return moves;
  ...
  private getSimpleMovesForPiece(
    board: Board,
    r: number,
    c: number,
    piece: CheckersPiece,
