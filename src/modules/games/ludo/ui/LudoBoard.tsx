...
export const LudoBoard: React.FC<LudoBoardProps> = ({ gameState, onAction, localUserId }) => {
  ...
  const renderGrid = () => {
    const cells = [];
    for (let row = 6; row < 9; row++) for (let col = 0; col < 15; col++) {
        if (col < 6 || col > 8) cells.push(<Cell key={`c-${row}-${col}`} r={row} c={col} type={getCellType(row, col)} />);
        else if (row !== 7 || col === 7) cells.push(<Cell key={`c-${row}-${col}`} r={row} c={col} type={getCellType(row, col)} />);
    }
    for (let col = 6; col < 9; col++) for (let row = 0; row < 15; row++) {
        if (row < 6 || row > 8) cells.push(<Cell key={`c-${row}-${col}`} r={row} c={col} type={getCellType(row, col)} />);
    }
    return cells;
  };
      ...
      <div className="relative w-full max-w-[92vh] aspect-square bg-[#0a0f1e] rounded-[3rem] p-6 shadow-2xl overflow-hidden group">
        ...
        <div className="relative w-full h-full bg-white rounded-[1.5rem] overflow-hidden shadow-[0_0_80px_rgba(0,0,0,0.6)]">
          <svg viewBox={`0 0 ${BOARD_SIZE} ${BOARD_SIZE}`} className="w-full h-full block">
              ...
              <path d={`M${6*UNIT},${6*UNIT} L${7.5*UNIT},${7.5*UNIT} L${9*UNIT},${6*UNIT} Z`} fill={COLORS.green} stroke="#fff" strokeWidth="4"/>
              <path d={`M${9*UNIT},${6*UNIT} L${7.5*UNIT},${7.5*UNIT} L${9*UNIT},${9*UNIT} Z`} fill={COLORS.yellow} stroke="#fff" strokeWidth="4"/>
              <path d={`M${9*UNIT},${9*UNIT} L${7.5*UNIT},${7.5*UNIT} L${6*UNIT},${9*UNIT} Z`} fill={COLORS.blue} stroke="#fff" strokeWidth="4"/>
              <path d={`M${6*UNIT},${9*UNIT} L${7.5*UNIT},${7.5*UNIT} L${6*UNIT},${6*UNIT} Z`} fill={COLORS.red} stroke="#fff" strokeWidth="4"/>
          </svg>
          ...
          <div className="absolute inset-0 z-50 pointer-events-none">
              <AnimatePresence>
                  ...
                  tokens?.[p.color!]?.map((pos, idx) => {
                      const canMove = diceRolled && currentTurn === localUserId && p.id === localUserId && 
                          ...
                          <Pawn 
                              ...
                              onClick={() => {
                                  if (canMove) {
                                      ...
                                      onAction?.({ type: 'MOVE_TOKEN', playerId: localUserId, tokenId: idx, fromCell: pos, toCell: nextPos });
                                  }
                              }}
                          />
                      );
...
const getCellType = (r: number, c: number): string => {
  ...
  const starCells = [
    {r: 8, c: 2}, {r: 2, c: 6}, {r: 6, c: 12}, {r: 12, c: 8}, // Standard Stars
    {r: 6, c: 2}, {r: 2, c: 8}, {r: 8, c: 12}, {r: 12, c: 6}  // Start Cells as safe spots (Audit A3)
  ];
...
const Cell = ({ r, c, type }: { r: number; c: number; type: string }) => {
  const fillMap: Record<string, string> = {
    normal: '#ffffff',
    'red-home': COLORS.red, 'green-home': COLORS.green, 'yellow-home': COLORS.yellow, 'blue-home': COLORS.blue,
    'red-start': COLORS.red, 'green-start': COLORS.green, 'yellow-start': COLORS.yellow, 'blue-start': COLORS.blue,
    'star': '#f8fafc',
  };
  const opacity = type.endsWith('-home') ? 0.35 : 1;
  return (
    <g transform={`translate(${c * UNIT}, ${r * UNIT})`}>
      <rect width={UNIT} height={UNIT} fill={fillMap[type] || '#fff'} fillOpacity={opacity} stroke="#e2e8f0" strokeWidth="2.5" />
      {type === 'star' && (
        <path d="M10,1 L13,7 L19,8 L15,13 L16,19 L10,16 L4,19 L5,13 L1,8 L7,7 Z"
          fill="#cbd5e1" transform={`translate(${UNIT*0.25}, ${UNIT*0.25}) scale(${UNIT/25})`} />
      )}
    </g>
  );
};
...
const House = ({ x, y, color }: { x: number; y: number; color: keyof typeof COLORS }) => {
  ...
  return (
    <g transform={`translate(${x * UNIT}, ${y * UNIT})`}>
      ...
      <circle cx={2*UNIT} cy={2*UNIT} r={UNIT*0.6} fill={hex} fillOpacity={0.12} stroke={hex} strokeWidth="3" />
      <circle cx={4*UNIT} cy={2*UNIT} r={UNIT*0.6} fill={hex} fillOpacity={0.12} stroke={hex} strokeWidth="3" />
      <circle cx={2*UNIT} cy={4*UNIT} r={UNIT*0.6} fill={hex} fillOpacity={0.12} stroke={hex} strokeWidth="3" />
      <circle cx={4*UNIT} cy={4*UNIT} r={UNIT*0.6} fill={hex} fillOpacity={0.12} stroke={hex} strokeWidth="3" />
    </g>
  );
};
