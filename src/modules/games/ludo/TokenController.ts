...
export class TokenController {
  ...
  private static getHomeStretchCoordinates(color: string, step: number): Point {
    ...
    const dist = step + 1;
    switch (color) {
        case 'red': return { x: dist + 0.5, y: 7.5 };
        case 'green': return { x: 7.5, y: dist + 0.5 };
        case 'yellow': return { x: 14.5 - dist, y: 7.5 };
        case 'blue': return { x: 7.5, y: 14.5 - dist };
        default: return { x: 7.5, y: 7.5 };
    }
  }
  ...
  static checkCaptures(globalIndex: number, attackerColor: string, tokens: Record<string, number[]>): Array<[string, number]> {
