    ...
    <div className='flex flex-col items-center justify-center min-h-screen bg-[#0a0f16] text-white p-4 font-sans select-none'>
      ...
      <div className='w-full max-w-4xl flex justify-between items-center mb-6 bg-white/5 backdrop-blur-md rounded-2xl p-4 border border-white/10 shadow-2xl'>
        <div className='flex items-center gap-4'>
          <div
            className={`p-3 rounded-xl ${gameState.currentPlayer === gameState.players[0].id ? 'bg-amber-500/20 border-amber-500/50' : 'bg-white/5'} border transition-all`}
          >
          ...
          <div
            className={`p-3 rounded-xl ${gameState.currentPlayer === gameState.players[1].id ? 'bg-indigo-500/20 border-indigo-500/50' : 'bg-white/5'} border transition-all`}
          >
            <p className='text-[10px] font-black uppercase tracking-widest text-white/40 mb-1'>
              Player 2
            </p>
...
function drawBoardPatterns(ctx: CanvasRenderingContext2D, state: CarromGameState) {
  ...
  [
    [baselineOffset, h - 110 + 16],
    [w - baselineOffset, h - 110 + 16],
    [baselineOffset, 110 - 16],
    [w - baselineOffset, 110 - 16],
  ].forEach(([x, y]) => {
    ctx.beginPath();
