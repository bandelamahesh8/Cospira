...
export class MetaEvolutionService {
    ...
    static async evolveMeta(gameId: string) {
        ...
        for (const strat of gameStrategies) {
             ...
             if (strat.win_rate > 0.60 && strat.usage_rate > 0.30) {
                 mutation = {
                     action: 'nerf',
    ...
    static async getPendingMutations() {
        const { data } = await supabase.from('meta_evolution')
            .select('*')
            .eq('status', 'PENDING');
    ...
    static async getMetaForecast() {
        ...
        return strategies.map(key => {
            ...
            return {
                ...
                prediction: currentWinRate + growthRate > 0.60 ? 'DOMINANT' : 
                           currentWinRate + growthRate < 0.40 ? 'OBSOLETE' : 'STABLE',
