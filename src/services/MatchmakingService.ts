import { ActionType, DecisionEngine } from './DecisionEngine';
...
export class MatchmakingService {
    ...
    static calculateMatchScore(p1: MatchCandidate, p2: MatchCandidate): number {
        ...
        if (p1.prediction && p1.prediction.tilt_probability > 0.6) {
             ...
             if (p2.elo < p1.elo) predictionScore = 1.0; // Good for tilt
     ...
     */
    static async findBestMatch(userId: string, currentElo: number, candidates: MatchCandidate[]): Promise<MatchCandidate | null> {
        // Hydrate P1
...
export class PublicMatchmakingService {
    ...
    private setupSocketHandlers() {
        this.socket?.on('public-match-found', (data: any) => {
            this.handleMatchFound(data);
        });
        this.socket?.on('rematch-requested', (data: any) => {
    ...
    async joinQueue(userId: string, userName: string, gameType: string): Promise<boolean> {
        try {
            this.socket?.emit('matchmaking-join-public', { userId, userName, gameType }, (response: any) => {
                if (response?.success) {
                    this.emit('queue-joined', { userId, gameType });
                } else {
                    this.emit('queue-error', response?.error);
                }
            });
            return true;
        } catch (error) {
    ...
    private handleMatchFound(data: any) {
        const { roomId, opponent, gameType } = data;
        this.currentMatch = {
            roomId,
            opponent,
            gameType,
