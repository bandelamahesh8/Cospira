...
export class ReinforcementService {
    ...
    static async simulateFeedbackLoop(userId: string, decisionType: string) {
        ...
        console.log(`[RL] Feedback for ${decisionType}: ${outcome} (Reward: ${reward})`);

        // 4. Store Experience
        const { data } = await supabase.from('brain_decision_outcomes').insert({
            user_id: userId,
    ...
    static async getLearningStats() {
        const { data } = await supabase.from('brain_decision_outcomes')
            .select('*')
