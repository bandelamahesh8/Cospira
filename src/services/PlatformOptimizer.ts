...
export class PlatformOptimizer {
    ...
    static async calculateHealth() {
        ...
        const fairness = 0.90 + (Math.random() * 0.05);
        ...
        let wRet = 0.4, wQual = 0.3, wFair = 0.2, wRev = 0.1;

        if (retention < 0.6) {
            console.log("⚠️ [OPTIMIZER] Retention Critical! Shifting weights.");
            wRet = 0.8; wQual = 0.1; wFair = 0.05; wRev = 0.05;
        }

        const revenueNorm = Math.min(revenue / 2000, 1.0);
        
        const score = (wRet * retention) + 
                      (wQual * quality) + 
                      (wFair * fairness) + 
                      (wRev * revenueNorm);
        ...
        // await supabase.from('platform_health_logs').insert({...health});
    ...
    static async runOptimization() {
        ...
        console.log("[OPTIMIZER] Testing config: Matchmaking_Strictness = HIGH");
