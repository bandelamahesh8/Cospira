...
export const ReplayViewer = ({ replayId, replayData, onClose }: ReplayViewerProps) => {
    ...
    return (
        <div className="flex flex-col items-center bg-slate-900 border border-slate-700 rounded-lg p-6 max-w-2xl mx-auto">
            <h2 className="text-2xl font-bold text-white mb-2">Replay: {replay.game_type.toUpperCase()}</h2>
            <div className="flex justify-between w-full text-slate-400 text-sm mb-4">
                <span>{replay.players.white?.name || 'Player 1'} vs {replay.players.black?.name || 'Player 2'}</span>
            ...
            <div className="w-full max-w-md aspect-square bg-slate-800 rounded-lg mb-6 flex items-center justify-center relative">
                 {replay.game_type === 'chess' ? (
                     ...
                     <div className="text-slate-500">
                         Visual replay only available for Chess currently.<br/>
            ...
            <div className="flex gap-4 items-center mb-4">
                <Button variant="outline" size="icon" onClick={() => setCurrentMoveIndex(-1)} disabled={currentMoveIndex < 0}>
                    <SkipBack className="w-4 h-4" />
                </Button>
                <Button variant="outline" size="icon" onClick={() => setCurrentMoveIndex(p => Math.max(-1, p - 1))} disabled={currentMoveIndex < 0}>
                    <ChevronLeft className="w-4 h-4" />
                </Button>
                
                <Button onClick={() => setIsPlaying(!isPlaying)} className={isPlaying ? "bg-amber-600" : "bg-green-600"}>
                    {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                </Button>

                <Button variant="outline" size="icon" onClick={() => setCurrentMoveIndex(p => Math.min(totalMoves - 1, p + 1))} disabled={currentMoveIndex >= totalMoves - 1}>
                    <ChevronRight className="w-4 h-4" />
                </Button>
                <Button variant="outline" size="icon" onClick={() => setCurrentMoveIndex(totalMoves - 1)} disabled={currentMoveIndex >= totalMoves - 1}>
            ...
            {onClose && <Button variant="ghost" className="mt-4" onClick={onClose}>Close Replay</Button>}
        </div>
    );
};
