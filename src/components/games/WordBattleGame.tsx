...
export const WordBattleGame = () => {
  ...
  const gameData = useMemo(
    () =>
      gameState && gameState.type === 'wordbattle'
        ? (gameState as unknown as WordBattleGameState)
        : null,
    [gameState]
  );
    ...
    <Card className='w-full max-w-4xl mx-auto bg-slate-900 border-4 border-indigo-500 shadow-[0_0_50px_rgba(99,102,241,0.5)] relative overflow-hidden flex flex-col p-8 min-h-[500px] rounded-[2rem]'>
      ...
      <div className='relative z-10 flex justify-between items-start mb-12'>
        <div className='space-y-1'>
          ...
          <p className='text-zinc-400 font-bold tracking-widest text-sm'>
            ROUND {gameData.round} / {gameData.maxRounds}
          </p>
        </div>
          ...
          {players.map((p) => (
            <div
              ...
              className={cn(
                ...
                gameData.scores[p.id] >
                  (gameData.scores[players.find((x) => x.id !== p.id)?.id || ''] || 0)
      ...
      <div className='flex-1 flex flex-col items-center justify-center relative z-10 gap-12'>
        ...
        >
          <p className='text-sm font-bold text-indigo-400 uppercase tracking-[0.3em] mb-4'>
            UNSCRAMBLE THIS
          </p>
        ...
        <form
          ...
          className={cn(
            'w-full max-w-md relative transition-transform',
