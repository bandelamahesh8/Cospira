...
export const CheckersGame = () => {
    ...
    const gameData = useMemo(() => (gameState && (gameState as any).type === 'checkers' ? gameState : null), [gameState]);
    const players = useMemo(() => gameData?.players || [], [gameData?.players]);
    const board = useMemo(() => (gameData?.board as ({ type: 'man' | 'king', color: 'red' | 'white' } | null)[][]) || [], [gameData?.board]);
    ...
    */
    ...
    // Simple sound trigger on turn change for now
    useEffect(() => {
        if (gameData?.metadata?.lastMove) {
             // Heuristic: dist > 1 is capture
             const { from, to } = (gameData.metadata.lastMove as any).data || {};
             if (from && to && Math.abs(from.r - to.r) > 1) playSound('capture');
             else playSound('move');
        }
    }, [gameData?.metadata?.moveHistory?.length, playSound, gameData?.metadata?.lastMove]);
        ...
        <Card className="w-full max-w-4xl mx-auto bg-slate-950 border-slate-800 shadow-[0_0_120px_rgba(0,0,0,0.9)] relative overflow-hidden flex flex-col md:flex-row p-6 md:p-10 rounded-[3rem] border-2">
            ...
            <div className="w-full md:w-80 flex flex-col gap-6 relative z-10 mt-8 md:mt-0 md:pl-8 border-l border-white/5">
                <div className="space-y-1">
                    <h1 className="text-3xl font-black text-white italic uppercase leading-none">Grand <span className="text-red-500">Checkers</span></h1>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Tactical Warfare</p>
                </div>
                ...
                <div className="p-6 bg-white/5 rounded-[2rem] border border-white/10 backdrop-blur-md shadow-xl flex flex-col items-center gap-6">
                    <div className={cn(
                        ...
                        <Users className={cn("w-10 h-10", players.find(p => p.id === turn)?.role === 'red' ? "text-white" : "text-slate-800")} />
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                     <div className="p-3 bg-red-900/20 border border-red-500/20 rounded-2xl text-center">
                        ...
                        <span className="text-2xl font-black text-white">{board.flat().filter(p => p?.color === 'red').length}</span>
                     </div>
                     <div className="p-3 bg-slate-800/50 border border-slate-700/50 rounded-2xl text-center">
                        ...
                        <span className="text-2xl font-black text-white">{board.flat().filter(p => p?.color === 'white').length}</span>
                     </div>
                </div>
            ...
            <GameResultOverlay 
                ...
                localUserId={user?.id || ''}
                isHost={isHost}
                onRematch={() => socket?.emit('start-game', { roomId, type: 'checkers', players: players.map(p => p.id) })}
                onEndGame={() => endGame()}
                gameType="checkers"
            />
        </Card>
