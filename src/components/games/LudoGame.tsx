...
export const LudoGame = () => {
  ...
  const [_activeReactions, setActiveReactions] = useState<
    { pIdx: number; emoji: string; id: number }[]
  >([]);
        ...
        >
          ...
          <div className='space-y-4'>
            ...
            <div className='inline-flex items-center gap-2 px-4 py-2 bg-purple-500/10 rounded-full border border-purple-500/20'>
              ...
              <span className='text-[10px] font-black text-purple-400 uppercase tracking-[.3em]'>
                Under Construction
              </span>
            </div>
          </div>
          ...
          <p className='text-slate-400 font-medium max-w-md mx-auto leading-relaxed uppercase tracking-widest text-[10px]'>
            The Ultimate Arena is currently being forged. <br /> Stay tuned for a high-fidelity
    ...
    <Card className='w-full max-w-5xl mx-auto bg-[#1a1a2e] border-[#8a2be2] shadow-[0_0_100px_rgba(138,43,226,0.3)] relative overflow-hidden flex flex-col lg:flex-row p-4 lg:p-8 rounded-[3rem] border-4'>
            ...
            <div className='w-[85%] h-[85%] bg-white rounded-full shadow-lg flex items-center justify-center relative'>
              <div className='grid grid-cols-2 gap-4'>
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className='w-8 h-8 rounded-full bg-yellow-500 shadow-md border-2 border-yellow-600/30'
                  />
                ))}
              </div>
            </div>
            ...
            <div className='w-[85%] h-[85%] bg-white rounded-full shadow-lg flex items-center justify-center relative'>
              <div className='grid grid-cols-2 gap-4'>
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className='w-8 h-8 rounded-full bg-red-600 shadow-md border-2 border-red-700/30'
                  />
                ))}
              </div>
            </div>
            ...
            <div className='w-[85%] h-[85%] bg-white rounded-full shadow-lg flex items-center justify-center relative'>
              <div className='grid grid-cols-2 gap-4'>
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className='w-8 h-8 rounded-full bg-blue-500 shadow-md border-2 border-blue-600/30'
                  />
                ))}
              </div>
            </div>
          ...
          {Array.from({ length: 225 }).map((_, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Path entry arrows */}
                {isGreenEnter && (
                  <div className='absolute inset-0 bg-emerald-500 flex items-center justify-center shadow-inner'>
                    <ChevronDown className='w-full h-full text-white stroke-[4]' />
                  </div>
                )}
                {isYellowEnter && (
                  <div className='absolute inset-0 bg-yellow-400 flex items-center justify-center shadow-inner'>
                    <ChevronLeft className='w-full h-full text-white stroke-[4]' />
                  </div>
                )}
                {isBlueEnter && (
                  <div className='absolute inset-0 bg-blue-500 flex items-center justify-center shadow-inner'>
                    <ChevronUp className='w-full h-full text-white stroke-[4]' />
                  </div>
                )}
                {isRedEnter && (
                  <div className='absolute inset-0 bg-red-500 flex items-center justify-center shadow-inner'>
                    <ChevronRight className='w-full h-full text-white stroke-[4]' />
                  </div>
                )}

                {/* Decorative Curved Arrows from Image */}
                {r === 7 && c === 1 && (
                  <ChevronRight className='absolute -left-1 w-4 h-4 text-slate-400 opacity-50' />
                )}
                {r === 1 && c === 9 && (
                  <ChevronDown className='absolute -top-1 w-4 h-4 text-slate-400 opacity-50' />
                )}
                {r === 9 && c === 15 && (
                  <ChevronLeft className='absolute -right-1 w-4 h-4 text-slate-400 opacity-50' />
                )}
                {r === 15 && c === 7 && (
                  <ChevronUp className='absolute -bottom-1 w-4 h-4 text-slate-400 opacity-50' />
                )}
              </div>
            );
          })}
                ...
                return (
                  ...
                  >
                    <motion.div
                      ...
                      transition={
                        isMovable ? { repeat: Infinity, duration: 1.5, ease: 'easeInOut' } : {}
                      }
                      className={cn(
      ...
      <div className='w-full lg:w-[400px] flex flex-col gap-6 relative z-10 pt-8 lg:pt-0'>
        <div className='space-y-2 text-center lg:text-left'>
          ...
          <h1 className='text-4xl font-black text-white tracking-widest leading-none uppercase'>
            Ludo <span className='text-[#8a2be2] drop-shadow-lg'>King</span>
          </h1>
          ...
          <AnimatePresence mode='wait'>
            {showLeaderboard ? (
              <motion.div
                ...
                className='p-6 bg-white/5 rounded-[2.5rem] border border-white/10 space-y-4 backdrop-blur-xl'
              >
                <h3 className='text-sm font-black text-[#8a2be2] uppercase tracking-widest mb-4'>
                  Current Standings
                </h3>
                {players
                  ...
                  .map((p, i) => (
                    <div
                      key={p.id}
                      className='flex items-center gap-4 p-3 rounded-2xl bg-white/5 border border-white/5 hover:border-[#8a2be2]/50 transition-all'
                    >
                      <div className='w-8 h-8 rounded-lg bg-indigo-500/20 flex items-center justify-center font-black text-xs text-indigo-400'>
                        {i + 1}
                      </div>
                      <p className='flex-1 font-bold text-white uppercase text-xs truncate'>
                        {p.name}
                      </p>
                ...
                <Button
                  variant='link'
                  className='text-slate-500 w-full mt-2 text-[10px]'
              ...
              >
                <div className='p-6 bg-white/5 rounded-[2.5rem] border border-white/10 shadow-2xl relative overflow-hidden group backdrop-blur-md'>
                  <div className='flex items-center gap-5 mb-6'>
                    <div
                      className={cn(
                        'w-16 h-16 rounded-[1.5rem] flex items-center justify-center shadow-xl border-t-2 border-white/20 transition-all',
                    ...
                    <div className='flex-1'>
                      <div className='flex items-center gap-2'>
                        <p className='text-xl font-black text-white truncate w-32'>
                          {(players.find((p) => p.id === turn) as Player)?.name.split(' ')[0] ||
                            'Player'}
                        </p>
                        {(gameData?.metadata as { isTeamMode?: boolean })?.isTeamMode && (
                          <span
                            className={cn(
                              'text-[9px] px-1.5 py-0.5 rounded font-black uppercase text-white',
                              players.findIndex((p) => p.id === turn) === 0 ||
                                players.findIndex((p) => p.id === turn) === 2
                                ? 'bg-orange-500'
                                : 'bg-cyan-500'
                            )}
                          >
                      ...
                      <div className='flex items-center gap-2 mt-1'>
                        <div
                          className={cn(
                            ...
                            isMyTurn ? 'bg-emerald-500 animate-pulse' : 'bg-slate-600'
                          )}
                        />
                        <span className='text-[10px] font-black text-slate-500 mb-0'>
                  ...
                  <div className='flex items-center justify-between gap-4'>
                    ...
                    {isMyTurn && phase === 'MOVE' && (
                      <p className='flex-1 text-center font-black animate-bounce text-[#8a2be2] text-4xl'>
                        {dice}
                      </p>
                    )}
                  </div>
                ...
                <Button
                  variant='outline'
                  className='w-full h-12 bg-white/5 border-white/10 text-slate-500 hover:text-white rounded-xl font-black uppercase tracking-widest text-[10px]'
                  onClick={() => endGame()}
                >
                  Surrender Match
                </Button>
      ...
      <GameResultOverlay
        ...
        isHost={isHost}
        onRematch={() =>
          socket?.emit('start-game', { roomId, type: 'ludo', players: players.map((p) => p.id) })
        }
        onEndGame={() => endGame()}
        gameType='ludo'
      />
