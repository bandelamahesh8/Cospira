...
export const SnakeLadderGame = () => {
    ...
    const gameData = useMemo(() => (gameState && gameState.type === 'snakeladder' ? gameState : null), [gameState]);
        ...
        players.forEach(p => {
            ...
            if (currentVisual !== target && !animatingRef.current[p.id]) {
                const moveStepByStep = async () => {
                    ...
                    const isMove = gameData?.lastAction?.type === 'MOVE' && gameData?.lastAction?.playerId === p.id;
        ...
        <Card className="w-full max-w-5xl mx-auto bg-slate-950 border-slate-800 shadow-[0_0_120px_rgba(0,0,0,0.9)] relative overflow-hidden flex flex-col lg:flex-row p-6 lg:p-10 rounded-[4rem] border-2">
            ...
            <div className="flex-1 relative aspect-square max-w-[550px] mx-auto perspective-2000 p-4">
                ...
                >
                    ...
                    >
                        {board?.snakes && Object.entries(board.snakes).map(([f, t]) => renderConnection(Number(f), Number(t), '#ef4444', true))}
                        {board?.ladders && Object.entries(board.ladders).map(([f, t]) => renderConnection(Number(f), Number(t), '#22c55e', false))}
                    </svg>
                    ...
                    <div className="absolute inset-0 w-full h-full pointer-events-none">
                        {players.map((p, idx) => {
                            ...
                            const samePos = players.filter((o, i) => (visualPositions[o.id] || o.pos || 1) === vPos && i < idx).length;
            ...
            <div className="w-full lg:w-96 flex flex-col gap-8 relative z-10 pt-10 lg:pt-0">
                <div className="space-y-2 text-center lg:text-left">
                    <div className="inline-flex items-center gap-3 px-4 py-2 bg-emerald-500/10 rounded-2xl border border-emerald-500/20 mb-4">
                        ...
                        <span className="text-xs font-black text-emerald-400 uppercase tracking-widest text-[10px]">Grand Duel</span>
                    </div>
                    <h1 className="text-4xl font-black text-white tracking-tighter italic uppercase leading-none">Snakes <br/> <span className="text-emerald-500 underline decoration-8 decoration-emerald-500/20 underline-offset-8">Arena</span></h1>
                </div>

                <div className="flex-1 space-y-6">
                    <div className="p-8 bg-white/5 rounded-[3rem] border border-white/10 backdrop-blur-3xl shadow-2xl space-y-8">
                        <div className="flex items-center gap-5">
                            <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center shadow-xl transition-transform", 
                                players.find(p => p.id === turn)?.color === 'red' ? 'bg-red-500 shadow-red-500/20' :
                                players.find(p => p.id === turn)?.color === 'green' ? 'bg-emerald-500 shadow-emerald-500/20' :
                                players.find(p => p.id === turn)?.color === 'yellow' ? 'bg-yellow-500 shadow-yellow-500/20' : 'bg-blue-500 shadow-blue-500/20'
                            )}>
                                <Users className="w-7 h-7 text-white" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-xl font-black text-white truncate">{players.find(p => p.id === turn)?.name || "Player"}</p>
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{isMyTurn ? "Your Turn" : "Waiting..."}</p>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col items-center gap-6">
                            ...
                            {isMyTurn && phase === 'ROLL' && (
                                <div className="flex gap-2 w-full justify-center">
                                    {(gameData?.metadata?.playerPowerUps?.[user?.id || ''] || []).map((pu: any) => (
                                        <Button
                                            key={pu.id}
                                            disabled={pu.count <= 0}
                                        ...
                                        >
                                            <span className="text-[10px] font-black uppercase text-white/70">{pu.name}</span>
                            ...
                            {isMyTurn && (
                                <Button 
                                    ...
                                    className={cn(
                                        ...
                                        phase === 'ROLL' ? "bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/20" : "bg-blue-600 hover:bg-blue-500 shadow-blue-600/20 animate-pulse"
                                    )}
                                >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <Button variant="outline" onClick={() => setIsBoardTilted(!isBoardTilted)} className="h-14 bg-white/5 border-white/10 text-slate-400 rounded-2xl hover:bg-white/10 hover:text-white transition-all text-xs font-black uppercase tracking-widest">
                            <Monitor className="w-4 h-4 mr-2" /> 3D View
                        </Button>
                        <Button variant="outline" onClick={() => endGame()} className="h-14 bg-white/5 border-white/10 text-slate-400 rounded-2xl hover:bg-red-500/20 hover:text-red-400 transition-all text-xs font-black uppercase tracking-widest">
            ...
            <GameResultOverlay 
                ...
                localUserId={user?.id || ''}
                isHost={isHost}
                onRematch={() => socket?.emit('start-game', { roomId, type: 'snakeladder', players: players.map(p => p.id) })}
                onEndGame={() => endGame()}
