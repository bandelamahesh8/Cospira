...
import { User as UserIcon, X, Check, AlertTriangle, Users, Loader2, Sparkles, Zap } from 'lucide-react';
...
export const GameHubModal = ({ open, onOpenChange, isPrivate = false }: { open: boolean, onOpenChange: (open: boolean) => void, isPrivate?: boolean }) => {
    ...
    const handleStartGame = () => {
        ...
        if (selectedOpponents.length === 0 || selectedOpponents.length > maxSlots) {
            toast.error(selectedOpponents.length === 0 ? 'Select at least one opponent.' : `This game allows up to ${maxSlots} opponent${maxSlots === 1 ? '' : 's'}.`);
            return;
        }
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-[90vw] md:max-w-[1200px] h-[85vh] p-0 bg-[#05080f] border border-white/10 rounded-[2.5rem] overflow-hidden flex flex-col focus:outline-none shadow-[0_50px_150px_rgba(0,0,0,0.8)] outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 slide-in-from-left-1/2 slide-in-from-top-[48%] duration-300">
                ...
                <div className="absolute top-0 left-0 right-0 z-50 flex justify-center pt-8 pointer-events-none">
                     <div className="pointer-events-auto flex items-center justify-between w-[95%] max-w-[1000px] h-14 px-6 glass-panel rounded-2xl border border-white/5 backdrop-blur-2xl bg-[#0b1220]/80 shadow-2xl">
                         {/* Left: Brand */}
                         <div className="flex items-center gap-4">
                            <div className="w-10 h-10 bg-gradient-to-br from-purple-600 to-blue-600 rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(124,58,237,0.5)]">
                                <Sparkles className="w-5 h-5 text-white animate-pulse" />
                            </div>
                            <div className="flex flex-col">
                                <span className="text-white font-black tracking-tighter uppercase text-lg leading-none">Cospira</span>
                                <span className="text-[10px] text-primary font-bold tracking-[0.3em] uppercase">Gaming OS</span>
                            </div>
                            <div className="h-8 w-px bg-white/10 mx-2" />
                            <nav className="flex gap-1">
                                {['Home', 'Competitive', 'Arcade', 'Tournaments'].map((item, i) => (
                                    <button 
                                        key={item} 
                                        onMouseEnter={() => playHover()}
                                        onClick={() => playClick()}
                                        className={`px-4 py-1.5 rounded-lg text-[10px] uppercase font-bold tracking-wider transition-all hover:bg-white/10 ${i === 0 ? 'bg-white/10 text-white' : 'text-white/40'}`}
                                    >
                                        {item}
                                    </button>
                                ))}
                         ...
                         <div className="flex items-center gap-6">
                            <div className="flex items-center gap-2">
                                <div className={`w-2 h-2 rounded-full shadow-sm ${hasMinMembers ? 'bg-green-500 animate-pulse shadow-[0_0_10px_#22c55e]' : 'bg-amber-500'}`} />
                                <span className={`text-[10px] font-bold uppercase tracking-wider ${hasMinMembers ? 'text-green-400' : 'text-amber-400'}`}>
                                    {users.length} Active Agent{users.length !== 1 ? 's' : ''}
                                    {!hasMinMembers && ' · Min 2 to play'}
                                </span>
                            </div>
                             <div className="flex items-center gap-3 pl-6 border-l border-white/10">
                                 <div className="text-right">
                                     <div className="text-xs font-bold text-white uppercase">{user?.user_metadata?.display_name || 'Unknown Agent'}</div>
                                     <div className="text-[9px] text-purple-400 font-bold uppercase tracking-wider">Level 42</div>
                                 </div>
                                 <div className="w-10 h-10 rounded-full bg-[#1a1f2e] border border-white/10 overflow-hidden">
                                     {user?.user_metadata?.photo_url ? (
                                         <img src={user.user_metadata.photo_url} alt="Profile" className="w-full h-full object-cover" />
                                     ) : (
                                         <UserIcon className="w-full h-full p-2 text-white/40" />
                                     )}
                                 </div>
                             </div>
                             <button onClick={() => { playClick(); onOpenChange(false); }} onMouseEnter={() => playHover()} className="w-10 h-10 rounded-full bg-white/5 hover:bg-red-500/20 hover:text-red-500 transition-all flex items-center justify-center">
                                 <X className="w-5 h-5" />
                             </button>
                         </div>
                     </div>
                ...
                {/* MAIN SCROLL CONTENT */}
                <div className="flex-1 w-full h-full overflow-y-auto overflow-x-hidden pt-24 pb-24 custom-scrollbar bg-[#05080f]">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {availableGames.map((game) => (
                                <motion.div 
                                    ...
                                    onMouseEnter={() => playHover()}
                                    onClick={() => { playClick(); setSelectedGame(game.id as any); }}
                                ...
                                >
                                    ...
                                    <div className="absolute inset-0">
                                        <img src={game.poster} alt={game.name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                                    ...
                                    <div className="absolute inset-x-0 bottom-0 p-5 flex flex-col justify-end h-full">
                                        <div className="mb-auto flex justify-end">
                                            {game.id === 'chess' && (
                                                 <span className="px-2 py-1 rounded bg-yellow-500 text-black text-[10px] font-black uppercase tracking-widest shadow-lg">Premium</span>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            <div className="flex gap-2 mb-2 flex-wrap">
                                                {game.tags?.slice(0, 2).map(tag => (
                                                    <span key={tag} className="text-[9px] font-bold uppercase tracking-wider bg-white/20 backdrop-blur px-2 py-0.5 rounded text-white">{tag}</span>
                                                ))}
                                            </div>
                                            <h3 className="text-2xl font-black text-white uppercase italic tracking-tight leading-none mb-1">{game.name}</h3>
                                            <p className="text-[10px] text-white/60 font-medium line-clamp-1 mb-3">{game.desc}</p>
                        ...
                        >
                            ...
                            >
                                ...
                                <div className="h-32 relative overflow-hidden">
                                    ...
                                    <img src={getCurrentGame()?.poster} className="w-full h-full object-cover opacity-40 mix-blend-overlay" />
                                    <div className="absolute inset-0 pl-8 flex items-center justify-between pr-16 w-full">
                                        <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter drop-shadow-xl">{getCurrentGame()?.name}</h2>
                                        {isPrivate && (
                                            <div className="px-4 py-1.5 bg-amber-500/20 border border-amber-500/50 rounded-full flex items-center gap-2">
                                                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse" />
                                                <span className="text-[10px] font-black text-amber-400 uppercase tracking-widest">Private Mode</span>
                                            </div>
                                        )}
                                    </div>
                                    <button 
                                ...
                                <div className="p-8 space-y-6">
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between">
                                            <label className="text-xs font-bold text-white/50 uppercase tracking-widest">Target Selection</label>
                                            <span className="text-xs font-bold text-purple-400 uppercase tracking-widest">{selectedOpponents.length} / {selectedGame ? getMaxOpponentSlots(selectedGame) : 0} Slots</span>
                                        </div>
                                        {!hasMinMembers && (
                                        ...
                                        {users.filter(u => u.id !== user?.id).length > 0 ? (
                                                ...
                                                {users.filter(u => u.id !== user?.id).map(u => {
                                                    ...
                                                    return (
                                                        <div 
                                                            ...
                                                            onClick={() => {
                                                                if (isSelected) {
                                                                    setSelectedOpponents(prev => prev.filter(id => id !== u.id));
                                                                } else {
                                                                    if (maxSlots === 1) {
                                                                       playClick();
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isSelected ? 'bg-purple-500 text-white' : 'bg-white/10 text-white/40'}`}>
                                                                    {isSelected ? <Check className="w-4 h-4" /> : <UserIcon className="w-4 h-4" />}
                                                                </div>
                                                                <div>
                                                                    <div className="text-sm font-bold text-white uppercase tracking-wide">{u.name}</div>
                                                                    <div className="text-[9px] text-white/30 uppercase tracking-widest font-medium">Ready for combat</div>
                                                                </div>
                                                            </div>
                                                            {isSelected && <div className="w-2 h-2 rounded-full bg-purple-500 shadow-[0_0_10px_#a855f7]" />}
                                                        </div>
                                                    );
                                                })}
                                            </div>
                                            ...
                                            <div className="p-8 border border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center text-center">
                                                ...
                                                <p className="text-sm text-white/40 font-medium">No active agents in lobby</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <button 
                                        ...
                                        disabled={!selectedGame || selectedOpponents.length === 0 || isEstablishing || !hasMinMembers}
...
export const AbandonGameModal = ({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) => {
        ...
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="luxury-glass border-white/5 p-12 rounded-[3.5rem] bg-black/80 backdrop-blur-3xl shadow-[0_50px_100px_rgba(0,0,0,0.8)] border-gradient">
                ...
                <AlertDialogFooter className="gap-4">
                    <AlertDialogCancel className="h-16 rounded-3xl border-white/10 bg-white/5 text-white/50 uppercase font-black text-[10px] tracking-widest hover:bg-white/10">STAY IN BATTLE</AlertDialogCancel>
                    <AlertDialogAction 
                        onClick={() => {
                            endGame();
                            onOpenChange(false);
