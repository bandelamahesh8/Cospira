...
export const UltimateTicTacToe = () => {
  ...
  const gameData = useMemo(
    () => (gameState && gameState.type === 'ultimate-xoxo' ? gameState : null),
    [gameState]
  );
  ...
  const board = useMemo(
    () =>
      (gameData?.board as UltimateBoard) || {
        cells: Array(9)
          .fill(null)
  ...
  const handleCellClick = (boardIndex: number, cellIndex: number) => {
    ...
    if (board.activeBoxIndex !== null && board.activeBoxIndex !== boardIndex) {
      toast({
        title: 'Invalid Board',
        description: 'You must play in the highlighted mini-board!',
        variant: 'destructive',
      });
      return;
    }
            ...
            >
              <div
                className={cn(
                  ...
                  p.role === 'X' ? 'bg-blue-600' : 'bg-rose-600'
                )}
              >
                {p.role === 'X' ? (
              ...
              <span className='text-[10px] font-black uppercase text-slate-400 max-w-[60px] truncate'>
                {p.name}
              </span>
            </div>
          ))}
        ...
        {Array(9)
          ...
          .map((_, boardIdx) => {
            ...
            const isActiveTarget =
              !winner &&
              (board.activeBoxIndex === null || board.activeBoxIndex === boardIdx) &&
              board.macroBoard[boardIdx] === null;
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  isActiveTarget
                    ? 'bg-amber-500/10 ring-2 ring-amber-500/50 shadow-[0_0_30px_rgba(245,158,11,0.2)]'
                    : 'bg-slate-900/40',
                  isWon ? 'opacity-90' : ''
                )}
              >
                ...
                {isWon && (
                  <div
                    className={cn(
                      'absolute inset-0 flex items-center justify-center bg-slate-950/80 backdrop-blur-[2px] rounded-xl border-2 z-10',
                      isWon === 'X'
                        ? 'border-blue-500/50'
                        : isWon === 'O'
                          ? 'border-rose-500/50'
                  ...
                  >
                    {isWon === 'X' && (
                      <X className='w-16 h-16 text-blue-500 drop-shadow-[0_0_10px_rgba(59,130,246,0.8)]' />
                    )}
                    {isWon === 'O' && (
                      <Circle className='w-16 h-16 text-rose-500 drop-shadow-[0_0_10px_rgba(244,63,94,0.8)]' />
                    )}
                  </div>
                )}
              </motion.div>
