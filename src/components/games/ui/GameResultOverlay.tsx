...
export const GameResultOverlay: React.FC<GameResultOverlayProps> = ({
    ...
    const winnerName = isDraw ? 'Everyone' : players.find(p => p.id === winnerId)?.name || 'Unknown User';
        ...
        >
            ...
            >
                ...
                <div className={cn(
                    ...
                    isWinner ? "bg-gradient-to-br from-yellow-400 to-amber-600 shadow-yellow-500/20" : 
                    isDraw ? "bg-gradient-to-br from-blue-400 to-indigo-600 shadow-blue-500/20" : 
                    "bg-gradient-to-br from-slate-600 to-slate-800 shadow-slate-900/20"
                )}>
                    {isWinner ? <Trophy className="w-14 h-14 text-white -rotate-12" /> : 
                     isDraw ? <Split className="w-14 h-14 text-white -rotate-12" /> : 
                ...
                <p className="text-slate-500 font-bold uppercase tracking-[0.3em] mb-12 text-sm leading-relaxed">
                    {isWinner ? "Master of the Arena" : 
                     isDraw ? "Neither side yielded" : 
                ...
                <div className="mt-10 pt-8 border-t border-white/5 relative z-10">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 mb-6">Match Standings</h3>
                    <div className="space-y-3">
                        {players.map((p) => (
                            <div key={p.id} className={cn(
                                "flex items-center justify-between p-3 rounded-2xl border transition-all",
                                p.id === winnerId ? "bg-primary/10 border-primary/20" : "bg-white/5 border-white/5"
                            )}>
