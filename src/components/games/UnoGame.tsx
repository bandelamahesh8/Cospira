...
import { UnoGameState, UnoCard } from '@/game-engine/engines/UnoEngine';
import { type GameState as WSGameState } from '@/types/websocket';
...
export const UnoGame = () => {
  ...
  const gameData = useMemo(
    () =>
      gameState && (gameState as WSGameState).type === 'uno'
        ? (gameState as unknown as UnoGameState)
        : null,
    [gameState]
  );
    ...
    <Card className='w-full max-w-6xl mx-auto bg-[#1a0b2e] border-purple-900/50 shadow-[0_0_100px_rgba(100,0,255,0.2)] relative overflow-hidden flex flex-col p-4 min-h-[600px] rounded-[3rem]'>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isTurn ? 'scale-110' : 'opacity-70'
                )}
              >
                <div
                  className={cn(
                    'w-16 h-16 rounded-full border-4 overflow-hidden relative shadow-xl',
                    isTurn ? 'border-yellow-400' : 'border-white/10'
                  )}
                >
                  <img
                    src={p.photoUrl || `https://api.dicebear.com/7.x/avataaars/svg?seed=${p.id}`}
                    alt={p.name}
      ...
      <div className='flex-1 flex items-center justify-center gap-12 relative z-10 my-8'>
        ...
        <motion.button
          ...
          disabled={!isMyTurn}
          className={cn(
            'w-32 h-48 bg-slate-900 rounded-2xl border-4 border-slate-700 shadow-2xl relative group',
        ...
        >
          ...
          {isMyTurn && (
            <div className='absolute -top-12 left-0 right-0 text-center text-white font-bold animate-bounce'>
              DRAW
            </div>
          )}
        </motion.button>
        ...
        <div className='relative w-32 h-48'>
          ...
          {topCard?.color === 'wild' && (
            <div
              className={cn(
                'absolute -bottom-8 left-0 right-0 h-4 rounded-full shadow-[0_0_20px_CURRENTCOLOR]',
                currentColor === 'red'
      ...
      <div className='relative z-10 pt-10 pb-4'>
          ...
          {myHand.map((card, i) => (
            <motion.div
              ...
              className={cn(
                ...
                !isMyTurn && 'opacity-70 pointer-events-none'
              )}
            >
        ...
        {isMyTurn && (
          <p className='text-center text-white font-black animate-pulse mt-2'>YOUR TURN</p>
        )}
      </div>
            ...
            <div className='grid grid-cols-2 gap-4 p-8'>
              {['red', 'blue', 'green', 'yellow'].map((c) => (
                <button
                  ...
                  className={cn(
                    'w-32 h-32 rounded-3xl shadow-2xl hover:scale-110 transition-all',
                    c === 'red'
                      ? 'bg-red-500'
                      : c === 'blue'
                        ? 'bg-blue-500'
      ...
      <GameResultOverlay
        ...
        isHost={isHost}
        onRematch={() =>
          socket?.emit('start-game', { roomId, type: 'uno', players: players.map((p) => p.id) })
        }
        onEndGame={() => endGame()}
        gameType='uno'
      />
...
const UnoCardView = ({ card }: { card: UnoCard }) => {
  if (!card)
    return <div className='w-32 h-48 bg-slate-800 rounded-2xl border-4 border-slate-700' />;
  ...
  return (
    <div
      className={cn(
        'w-32 h-48 rounded-2xl border-4 shadow-xl flex items-center justify-center relative overflow-hidden',
        colors[card.color]
      )}
    >
      ...
      <div className='w-24 h-36 border-2 border-white/30 rounded-[3rem] flex items-center justify-center -rotate-12 bg-black/10'>
        <span
          className={cn(
            'font-black text-5xl text-white drop-shadow-md',
            card.value.length > 2 && 'text-2xl'
          )}
        >
          {card.value === 'skip'
            ? '🚫'
      ...
      <span className='absolute bottom-2 right-3 font-bold text-white/80 rotate-180'>
        {card.value}
      </span>
    </div>
  );
