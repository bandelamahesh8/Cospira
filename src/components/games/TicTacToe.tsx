...
export const TicTacToe = () => {
    ...
    const gameData = useMemo(() => (activeGameState && activeGameState.type === 'xoxo' ? activeGameState : null), [activeGameState]);
    ...
    const scores = useMemo(() => (activeGameState as unknown as { scores?: Record<string, number> })?.scores || {}, [activeGameState]);
    ...
    useEffect(() => {
        if (winner) {
             ...
             const frame = () => {
                 confetti({ particleCount: 5, angle: 60, spread: 55, origin: { x: 0, y: 0.8 }, colors, zIndex: 10000 });
                 confetti({ particleCount: 5, angle: 120, spread: 55, origin: { x: 1, y: 0.8 }, colors, zIndex: 10000 });
             ...
             if (user?.id && gameData && !isVsAI) {
                 ...
                 const duration = Math.floor((Date.now() - (gameData.createdAt?.getTime() || Date.now())) / 1000);
                 
                 playerService.updateGameStats(user.id, 'xoxo', result, opponentElo, duration)
        ...
        const newGame: GameState = {
            ...
            players: [
                { 
                    ...
                    avatarUrl: user.user_metadata?.avatar_url || 'https://api.dicebear.com/7.x/avataaars/svg?seed=player' 
                },
                { 
                    id: 'ai-bot', 
    ...
    // AI Move Logic
    useEffect(() => {
        if (isVsAI && localGameState && !localGameState.winner && localGameState.currentTurn === 'ai-bot') {
    ...
    const handleRematch = () => {
        if (isVsAI) {
            ...
            socket?.emit('start-game', { roomId, type: 'xoxo', players: players.map(p => p.id), config: { isTimeAttack } });
        }
    };
        ...
        <Card className="w-full max-w-xl mx-auto bg-slate-950 border-slate-800 shadow-[0_0_100px_rgba(30,58,138,0.5)] relative overflow-hidden flex flex-col p-8 md:p-12 rounded-[4rem] border-2">
            ...
            <div className="relative z-10 flex justify-between items-center mb-12">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h2 className="text-4xl font-black text-white tracking-tighter italic uppercase leading-none">XOXO <span className="text-blue-500">PRO</span></h2>
                        <div className="flex items-center gap-2 mt-1">
                            <div className={cn("w-2.5 h-2.5 rounded-full", !winner && isMyTurn ? "bg-emerald-500 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "bg-slate-600")} />
                            <span className="text-xs font-black text-slate-500 uppercase tracking-widest">
                                {winner ? 'Match Concluded' : isMyTurn ? 'Your Strategic Move' : `${currentPlayerName.split(' ')[0]}'s Move`}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    {!isVsAI && !gameData && (
                        <Button
                            ...
                            className={cn(
                                ...
                                isTimeAttack ? "bg-red-600 hover:bg-red-700 border-red-500 animate-pulse" : "bg-white/5"
                            )}
                    ...
                    <Button 
                        ...
                        size="icon" 
                        onClick={startAIGame}
                        className={cn("w-12 h-12 rounded-2xl border-white/10 text-white hover:bg-white/10", isVsAI ? "bg-blue-600 hover:bg-blue-700 border-blue-400" : "bg-white/5")}
                ...
                <>
                    <div className="mb-2 relative h-2 bg-slate-800 rounded-full overflow-hidden">
                        <motion.div 
                            ...
                            className={cn("absolute h-full rounded-full", isMyTurn ? "bg-blue-500" : "bg-slate-500")}
                        />
                    </div>
            ...
            <div className="relative z-10 flex-1 min-h-[400px]">
                <AnimatePresence mode="wait">
                    {!showLeaderboard ? (
                        ...
                        >
                            {board.map((cell: string | null, i: number) => {
                                ...
                                return (
                                    <motion.button
                                        key={i}
                                        whileHover={!normalizedCell && !winner ? { scale: 1.05, backgroundColor: 'rgba(255,255,255,0.08)' } : {}}
                                        ...
                                        className={cn(
                                            ...
                                            !normalizedCell && !winner ? "bg-white/5 border-white/10 shadow-inner group" : "bg-slate-900 border-slate-700",
                                            normalizedCell === 'X' ? "bg-blue-600 border-blue-400 shadow-[0_0_40px_rgba(59,130,246,0.4)]" : 
                                            normalizedCell === 'O' ? "bg-purple-600 border-purple-400 shadow-[0_0_40px_rgba(168,85,247,0.4)]" : ""
                                        )}
                                    >
                                        <AnimatePresence mode="wait">
                                            {normalizedCell === 'X' && (
                        ...
                        >
                            <h3 className="text-2xl font-black text-white text-center mb-8 uppercase tracking-widest italic">Hall of Fame</h3>
                            {players.sort((a,b) => (scores[b.id]||0) - (scores[a.id]||0)).map((p, idx) => (
                                <div key={p.id} className="flex items-center gap-4 p-5 bg-white/5 rounded-3xl border border-white/10 group hover:bg-white/10 transition-all">
                                    <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center font-black", 
                                        idx === 0 ? "bg-yellow-500 text-black shadow-[0_0_15px_rgba(234,179,8,0.5)]" : "bg-slate-800 text-slate-400"
                                    )}>
                                        {idx + 1}
                                    </div>
                                    <div className="flex-1">
                                        <p className="text-lg font-bold text-white uppercase">{p.name}</p>
                                        <p className="text-[10px] text-slate-500 font-black tracking-widest">{p.id === user?.id ? "YOU" : "OPPONENT"}</p>
                                    </div>
                            ...
                            <Button variant="ghost" className="w-full text-slate-500 hover:text-white mt-10" onClick={() => setShowLeaderboard(false)}>BACK TO GAME</Button>
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
