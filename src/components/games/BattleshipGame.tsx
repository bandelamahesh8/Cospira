...
export const BattleshipGame = () => {
  ...
  const gameData = useMemo(
    () =>
      gameState && (gameState as unknown as { type: string }).type === 'battleship'
        ? gameState
        : null,
    [gameState]
  );
  ...
  const phase = useMemo(
    () => (gameData?.metadata as { phase?: string })?.phase || 'SETUP',
    [gameData?.metadata]
  );
  const board = useMemo(
    () => (gameData?.board as Record<string, PlayerBoard>) || {},
    [gameData?.board]
  );
  ...
  const handleSetupClick = (r: number, c: number) => {
    ...
    for (let i = 0; i < size; i++) {
      ...
      const overlap = placedShips.some((s) =>
        s.coords.some((coord) => coord.r === nr && coord.c === nc)
      );
      if (overlap) return;
  ...
  const handleAttack = (r: number, c: number) => {
    if (
      !isMyTurn ||
      !!winner ||
      opponentBoard?.grid[r][c] === 'hit' ||
      opponentBoard?.grid[r][c] === 'miss'
    )
      return;
      ...
      <div className='flex justify-between items-center mb-8 relative z-10'>
        <div className='space-y-1'>
          <h1 className='text-3xl font-black text-white italic uppercase leading-none'>
            Battleship <span className='text-cyan-500'>Command</span>
          </h1>
        ...
        {!isSetup && (
          <div
            className={cn(
              ...
              isMyTurn
                ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/20 animate-pulse shadow-[0_0_20px_rgba(34,211,238,0.2)]'
      ...
      <div className='flex flex-col lg:flex-row gap-8 lg:gap-16 items-start justify-center relative z-10'>
        ...
        <div className='flex-1 w-full max-w-[500px] flex flex-col gap-4'>
          <div className='flex items-center justify-between text-white/50 text-[10px] font-black uppercase tracking-widest px-2'>
            <span className='flex items-center gap-2'>
              <Anchor className='w-3 h-3' /> My Fleet Status
            </span>
            {isSetup && !isReady && (
              <div className='flex gap-2'>
                <Button
                  size='sm'
                  variant='outline'
                  className='h-6 text-[9px] border-white/10 bg-white/5'
                  onClick={() =>
          ...
          >
            ...
            {Array.from({ length: 100 }).map((_, i) => {
              ...
              const isLocalShip =
                isSetup &&
                placedShips.some((s) => s.coords.some((coord) => coord.r === r && coord.c === c));
          ...
          {isSetup && !isReady && (
            <div className='flex flex-col gap-2 p-4 bg-cyan-950/30 border border-cyan-500/20 rounded-xl'>
              <p className='text-cyan-400 text-xs font-bold uppercase tracking-wider flex items-center gap-2'>
                <Play className='w-3 h-3' /> Deploying: {currentShipType?.toUpperCase()} (
                {SHIP_SIZES[currentShipType]})
              </p>
              <p className='text-white/30 text-[10px]'>
                Tap grid to place. Use Rotate to change orientation.
              </p>

              {placedShips.length === 5 && (
                <Button
                  onClick={handleConfirmSetup}
                  className='w-full bg-cyan-500 hover:bg-cyan-400 text-black font-black uppercase tracking-widest mt-2'
                >
                  Confirm Deployment
                </Button>
              )}
        ...
        {!isSetup && (
          <div className='flex-1 w-full max-w-[500px] flex flex-col gap-4'>
            <div className='flex items-center justify-between text-white/50 text-[10px] font-black uppercase tracking-widest px-2'>
              <span className='flex items-center gap-2'>
                <Crosshair className='w-3 h-3' /> Tactical Radar
              </span>
