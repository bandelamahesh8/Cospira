...
export const BilliardsGame = () => {
  ...
  const gameData = useMemo(
    () => (gameState && gameState.type === 'billiards' ? (gameState as BilliardsGameState) : null),
    [gameState]
  );
  ...
  return (
    <Card className='w-full max-w-6xl mx-auto bg-slate-900 p-4 rounded-3xl border border-slate-700 shadow-2xl relative'>
          ...
          {players.map((p) => (
            <div
              ...
              className={cn(
                ...
                gameData.currentTurn === p.id ? 'bg-green-600' : 'bg-slate-800'
              )}
            >
              <p className='font-bold'>{p.name}</p>
              <p className='text-xs opacity-70 uppercase tracking-widest'>
                {gameData.activePlayerType === 'open'
                  ? 'OPEN'
                  : gameData.currentTurn === p.id
                    ? gameData.activePlayerType
                    : gameData.activePlayerType === 'solid'
                      ? 'stripe'
                      : 'solid'}
              </p>
            </div>
      ...
      <p className='text-center text-slate-500 text-xs mt-4'>
        Running Custom 2D Physics Engine (Client-Predicted)
      </p>
