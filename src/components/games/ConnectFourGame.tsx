...
export const ConnectFourGame = () => {
  ...
  const gameData = useMemo(
    () =>
      gameState && (gameState as GameState).type === 'connect4'
  ...
  const board = useMemo(
    () => (gameData?.board as ('red' | 'yellow' | null)[][]) || [],
    [gameData?.board]
  );
              ...
              >
                ...
                {Array.from({ length: 6 }).map((_, rowIdx) => {
                  ...
                  return (
                    <div
                      key={`${colIdx}-${rowIdx}`}
                      className='w-8 h-8 md:w-12 md:h-12 rounded-full bg-slate-900/80 shadow-inner overflow-hidden relative'
                    >
                      <AnimatePresence>
                        {cell && (
                          <motion.div
