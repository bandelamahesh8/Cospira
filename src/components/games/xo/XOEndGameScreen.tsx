...
export const XOEndGameScreen = ({ result, onRematch, onExit }: EndGameScreenProps) => {
  return (
    ...
    >
      ...
      >
        {result === 'win' && (
          <p className="text-white text-2xl font-light">You won</p>
        )}
        {result === 'loss' && (
          <p className="text-slate-400 text-2xl font-light">You lost</p>
        )}
        {result === 'draw' && (
      ...
      <motion.div
        ...
        animate={{ opacity: 1 }}
        transition={{ 
          delay: (XO_CONFIG.TIMING.SILENCE_AFTER_WIN_MS + XO_CONFIG.TIMING.END_SESSION_PAUSE_MS) / 1000 
        }}
