    ...
    return (
        <div className="p-8 max-w-6xl mx-auto text-white">
            ...
            <div className="flex justify-between items-center mb-8">
                ...
                {view === 'bracket' && (
                    <Button variant="outline" onClick={() => setView('list')}>Back to Lobby</Button>
                )}
                ...
                {view === 'list' && (
                    <Dialog>
                        ...
                        <DialogContent className="bg-slate-900 border-slate-700 text-white">
                            ...
                            />
                            <Button onClick={handleCreate} className="w-full bg-yellow-500 text-black">Create</Button>
                        </DialogContent>
                    </Dialog>
                )}
            </div>
            ...
            {view === 'list' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {tournaments.map(t => (
                        <Card key={t.id} className="bg-slate-900 border-slate-700 p-6 flex flex-col gap-4 hover:border-indigo-500 transition-colors">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h3 className="text-xl font-bold">{t.name}</h3>
                                    <div className="flex gap-2 mt-1">
                                        <span className="text-xs bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded uppercase">{t.gameType}</span>
                                        <span className={`text-xs px-2 py-0.5 rounded uppercase ${t.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-slate-700 text-slate-400'}`}>
                                            {t.status}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div className="mt-auto flex gap-2">
                                {t.status === 'registration' && (
                                    <>
                                        <Button className="flex-1" onClick={() => handleJoin(t.id)}>Join</Button>
                                        {/* Only creator should see this really */}
                                         {t.createdBy === user?.id && (
                                            <Button variant="secondary" className="px-3" onClick={() => handleStart(t.id)}>
                    ...
                    {tournaments.length === 0 && <p className="text-slate-500">No active tournaments. Create one!</p>}
                </div>
            ) : (
