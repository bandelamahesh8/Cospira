...
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs'; // Assuming these exist or use div
import { User, Palette, Image as ImageIcon, ShoppingCart, Check } from 'lucide-react';
        ...
        <div className="p-8 max-w-5xl mx-auto text-white flex gap-8">
            ...
            <div className="w-1/3 space-y-4">
                <Card className="bg-slate-900 border-slate-700 p-6 flex flex-col items-center sticky top-4">
                    ...
                    {/* Banner Preview */}
                    <div className={`w-full h-32 rounded-t-lg mb-[-40px] ${profile?.equipped_banner_id ? '' : 'bg-slate-800'}`}>
            ...
            <div className="w-2/3">
                 ...
                 <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                     {items.map(asset => (
                         <Card key={asset.id} className="bg-slate-900 border-slate-700 p-4 flex flex-col items-center hover:border-indigo-500/50 transition-all">
                             <div className="w-16 h-16 bg-slate-800 rounded-lg mb-3 overflow-hidden">
                                {asset.type === 'avatar' && <img src={asset.image_url} className="w-full h-full object-cover"/>}
                                {asset.type === 'frame' && <div className={`w-full h-full border-4 ${asset.image_url}`}></div>} 
                             </div>
                             
                             <h4 className="font-bold text-sm mb-1">{asset.name}</h4>
                             <span className={`text-xs px-2 py-0.5 rounded uppercase font-bold mb-3
                                ${asset.rarity === 'legendary' ? 'bg-yellow-500/20 text-yellow-500' : 
                                  asset.rarity === 'epic' ? 'bg-purple-500/20 text-purple-500' : 
                             ...
                             {activeTab === 'shop' ? (
                                 isOwned(asset.id) ? (
                                     <Button disabled variant="secondary" className="w-full h-8 text-xs">Owned</Button>
                                 ) : (
                                     <Button onClick={() => handleBuy(asset)} className="w-full h-8 text-xs bg-green-600 hover:bg-green-700">
                                         {asset.price_coins} G
                                     </Button>
                                 )
                             ) : (
                                 <Button onClick={() => handleEquip(asset)} variant="outline" className="w-full h-8 text-xs border-slate-600">
                                     Equip
                                 </Button>
                             )}
                         </Card>
