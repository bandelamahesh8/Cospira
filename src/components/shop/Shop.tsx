    ...
    return (
        <div className="p-8 max-w-6xl mx-auto">
             <div className="flex justify-between items-center mb-8">
                 <div>
                     <h1 className="text-4xl font-black text-white italic uppercase tracking-tighter">Premium Store</h1>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
                 {items.map((item) => {
                     ...
                     return (
                         <Card key={item.id} className="bg-slate-900 border-slate-700 overflow-hidden relative group hover:border-indigo-500 transition-all flex flex-col">
                             ...
                             <div className="p-4 flex-1 flex flex-col">
                                 <div className="mb-4">
                                     <div className="flex justify-between items-start">
                                        ...
                                        <Badge variant="outline" className="uppercase text-[10px] tracking-wider">{item.category}</Badge>
                                     </div>
                                 ...
                                 <div className="mt-auto space-y-2">
                                    ...
                                    {item.price_coins !== null && (
                                         <Button 
                                            className={cn("w-full font-bold", isOwned ? "bg-slate-800" : "bg-yellow-500 hover:bg-yellow-600 text-black")}
                                         ...
                                         >
                                            {isOwned ? 'Equipped' : (
                                                <div className="flex items-center gap-1">
                                                    {loadingId === item.id ? '...' : 'Buy for'} {item.price_coins} <Coins className="w-3 h-3 ml-1" />
                                                </div>
                                            )}
                                         </Button>
                                    )}
                                    ...
                                    {item.price_gems !== null && (
                                         <Button 
                                            className={cn("w-full font-bold", isOwned ? "bg-slate-800" : "bg-cyan-600 hover:bg-cyan-700 text-white")}
                                         ...
                                         >
                                             {isOwned ? 'Equipped' : (
                                                <div className="flex items-center gap-1">
                                                    {loadingId === item.id ? '...' : 'Buy for'} {item.price_gems} <Diamond className="w-3 h-3 ml-1" />
                                                </div>
                                             )}
                                         </Button>
                                    )}
