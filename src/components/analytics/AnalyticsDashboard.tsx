...
import { History, PlayCircle, Target, Zap, Activity, Clock } from 'lucide-react';
import {
  AreaChart,
  Area,
  XAxis,
...
export const AnalyticsDashboard = () => {
  ...
  const CustomTooltip = ({
    ...
    active?: boolean;
    payload?: { value: number; payload: { matchId: string } }[];
    label?: string;
  }) => {
      ...
      return (
        <div className='bg-[#0c1016]/90 border border-emerald-500/30 p-3 rounded-xl backdrop-blur-xl shadow-2xl'>
          <p className='text-emerald-400 font-bold text-xs uppercase tracking-widest mb-1'>
            {label || 'Match ' + payload[0].payload.matchId}
          </p>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4 mb-8'>
        {[
          ...
          },
          {
            ...
            value: loading
              ? '...'
              : RoomAnalyticsService.formatDuration(analytics?.totalTimeSpentSeconds || 0),
            icon: Clock,
          ...
          },
          {
            ...
            value: loading
              ? '...'
              : RoomAnalyticsService.formatDuration(analytics?.ultraModeTimeSeconds || 0),
            icon: Zap,
          ...
          >
            <div
              className={`absolute top-0 right-0 p-4 opacity-20 ${stat.color} group-hover:opacity-40 group-hover:scale-110 transition-all duration-500`}
            >
              <stat.icon className='w-12 h-12' />
            </div>
            <div className='relative z-10'>
              <p
                className={`text-[10px] font-black uppercase tracking-widest opacity-60 flex items-center gap-2 mb-2 ${stat.color}`}
              >
            ...
            <div className='flex items-center justify-between mb-8'>
              <div>
                ...
                <p className='text-white/30 text-xs font-bold uppercase tracking-widest mt-1'>
                  Chess ELO Progression • Last 30 Days
                </p>
              </div>
              <div className='flex gap-2'>
                {['1W', '1M', '3M', 'ALL'].map((period) => (
                  <button
                    key={period}
                    className={`px-3 py-1 rounded-lg text-[10px] font-bold ${period === '1M' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-white/5 text-white/30 hover:bg-white/10'}`}
                  >
                    {period}
                  </button>
                ))}
        ...
        >
          ...
          <div className='flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 max-h-[400px] lg:max-h-none'>
            {recentMatches.map((match, i) => (
              ...
              >
                <div className='flex items-center gap-4'>
                  <div
                    className={`w-2 h-10 rounded-full ${match.winnerId === user?.id ? 'bg-emerald-500' : 'bg-red-500'}`}
                  />
                  <div>
                    ...
                    <div className='flex items-center gap-2'>
                      <span
                        className={`text-sm font-black uppercase ${match.winnerId === user?.id ? 'text-emerald-400' : 'text-red-400'}`}
                      >
                      ...
                      <span className='text-white/40 text-xs font-bold'>
                        vs {match.players?.find((p) => p.id !== user?.id)?.username || 'Opponent'}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            {recentMatches.length === 0 && (
              <p className='text-center text-white/20 text-xs py-8 font-bold uppercase tracking-widest'>
