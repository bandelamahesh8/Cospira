...
const NAV_ITEMS = [
  ...
  { icon: Rocket, label: 'Upcoming', path: '/upcoming' },
];
...
export const Sidebar = () => {
  ...
  return (
    <div className="h-full flex flex-col pt-6 relative overflow-hidden">
      ...
      <div className={`px-4 mb-6 flex items-center relative z-10 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
      ...
      <nav className={`flex-1 flex flex-col space-y-3 relative z-10 ${isCollapsed ? 'px-3' : 'px-4'}`}>
          ...
          {NAV_ITEMS.map((item) => {
            ...
            const ButtonContent = (
              ...
              >
                ...
                {isActive && (
                   <div 
                      className="absolute inset-0 rounded-2xl border border-blue-500/30 shadow-[0_0_20px_rgba(37,99,235,0.15)]"
                   />
                )}
            ...
            if (isCollapsed) {
              return (
                <Tooltip key={item.path}>
                  ...
                  <TooltipContent side="right" className="bg-[#0c1016] text-white border-white/10 font-bold ml-6 px-4 py-2 rounded-xl shadow-2xl backdrop-blur-xl">
                    {item.label}
                  </TooltipContent>
                </Tooltip>
              );
      ...
      {/* Bottom Section - About / Settings / System */}
      <div className={`pt-6 border-t border-white/5 space-y-3 pb-8 relative z-10 ${isCollapsed ? 'px-3' : 'px-4'}`}>
        <TooltipProvider delayDuration={0}>
          {/* About */}
          <Tooltip>
            <TooltipTrigger asChild>
              <motion.button
                ...
                className={`
                  w-full h-12 rounded-2xl flex items-center transition-all relative group
                  ${location.pathname === '/about' ? 'bg-blue-600/10' : 'hover:bg-white/5'}
                  ${isCollapsed ? 'justify-center px-0' : 'px-4 gap-4'}
                `}
              >
                ...
                <div className={`relative z-10 flex items-center ${isCollapsed ? 'justify-center' : 'gap-4'}`}>
                  <Info className={`w-5 h-5 shrink-0 transition-colors ${location.pathname === '/about' ? 'text-blue-400' : 'text-zinc-500 group-hover:text-white'}`} />
                  {!isCollapsed && (
                    <span className={`text-sm font-bold transition-colors whitespace-nowrap uppercase tracking-widest ${location.pathname === '/about' ? 'text-blue-100' : 'text-zinc-500 group-hover:text-white'}`}>
                      About
                    </span>
                  )}
                </div>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                ...
                <div className={`relative z-10 flex items-center ${isCollapsed ? 'justify-center' : 'gap-4'}`}>
                  <Settings className={`w-5 h-5 shrink-0 transition-colors ${location.pathname === '/settings' ? 'text-blue-400' : 'text-zinc-500 group-hover:text-white'}`} />
                  {!isCollapsed && (
                    <div className="flex flex-col items-start leading-none gap-0.5">
                      <span className={`text-sm font-bold transition-colors whitespace-nowrap uppercase tracking-widest ${location.pathname === '/settings' ? 'text-blue-100' : 'text-zinc-500 group-hover:text-white'}`}>
                        System
                      </span>
                      <span className="text-[9px] font-black text-emerald-500/60 uppercase tracking-[0.2em]">Optimal</span>
                    </div>
                  )}
                </div>
              </motion.button>
