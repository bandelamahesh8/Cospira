...
const NAV_ITEMS = [
  ...
  { icon: <Bot className="w-5 h-5" />, label: 'AI Interview', path: '/upcoming-features', badge: 'Upcoming' },
];
                ...
                <SidebarLink
                  key={item.path}
                   ...
                   className={cn(
                     ...
                     isActive ? 'bg-indigo-600/20 border-indigo-500/30 shadow-[0_4px_12px_rgba(99,102,241,0.1)]' : 'hover:bg-white/[0.03] hover:border-white/5'
                   )}
                />
            );
      ...
      <div className="pt-6 border-t border-white/5 space-y-4 pb-4 relative z-10">
         <SidebarLink
            link={{
                ...
                href: '/settings',
                icon: <Settings className={`w-5 h-5 shrink-0 transition-all duration-300 ${location.pathname === '/settings' ? 'text-white' : 'text-zinc-500 group-hover/sidebar:text-zinc-200'}`} />
            }}
            className={cn(
              "transition-all duration-300",
              location.pathname === '/settings' ? 'bg-indigo-600/20 border-indigo-500/30' : 'hover:bg-white/[0.03] hover:border-white/5'
            )}
         />
      </div>
    </div>
