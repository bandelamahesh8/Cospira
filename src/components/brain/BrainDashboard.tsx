...
import { type SocialNode } from '@/services/SocialGraphService';
import { Brain, Activity, Shield, Zap, Globe, FlaskConical, TrendingUp } from 'lucide-react';
...
export const BrainDashboard = () => {
  ...
  const [meta, setMeta] = useState<MetaStrategy[]>([]);
  ...
  const [_forecast, setForecast] = useState<unknown[]>([]);
  ...
  const loadIntel = useCallback(async () => {
    ...
    try {
      ...
      if (mut) {
        setMutations(mut);
        if (mut.length > 0) {
          await BrainControlService.executeAction(
            'APPLY_MUTATION',
            mut[0].strategy_key,
            mut[0].mutation_suggestion
          );
        }
      }
    ...
    <div className='p-6 bg-slate-950 min-h-screen text-slate-200 font-sans selection:bg-purple-500/30'>
      <div className='max-w-7xl mx-auto space-y-6'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                  <div className='bg-black/30 p-3 rounded-lg border border-purple-500/20'>
                    ...
                    <div
                      className={cn(
                        'text-2xl font-mono',
                        predictions.tilt_probability > 0.5 ? 'text-red-500' : 'text-green-500'
                      )}
                    >
                  ...
                  <div className='bg-black/30 p-3 rounded-lg border border-purple-500/20'>
                    ...
                    <div
                      className={cn(
                        'text-2xl font-mono',
                        predictions.churn_probability > 0.5 ? 'text-red-500' : 'text-green-500'
                      )}
                    >
                  ...
                  <div className='flex justify-between text-xs font-bold uppercase tracking-wider text-slate-500'>
                    ...
                    <div className='text-right'>
                      <span className='text-purple-400'>
                        {(predictions.win_probability * 100).toFixed(1)}%
                      </span>
                      {distributions && (
          ...
          <Card
            className={cn(
              'p-6 relative overflow-hidden border transition-all duration-500',
              decision?.action !== 'NONE'
                ? 'bg-red-950/20 border-red-500/50 shadow-[0_0_30px_rgba(239,68,68,0.1)]'
                : 'bg-slate-900 border-emerald-500/30'
            )}
          >
            <div
              className={cn(
                'absolute top-0 left-0 w-full h-1 shadow-[0_0_10px]',
                decision?.action !== 'NONE'
            ...
            <h2 className='text-xl font-bold mb-4 flex items-center gap-2'>
              <Zap
                className={cn(
                  'w-5 h-5',
                  decision?.action !== 'NONE' ? 'text-red-400' : 'text-emerald-400'
                )}
              />
                ...
                <div>
                  ...
                  <div
                    className={cn(
                      ...
                      decision.action !== 'NONE' ? 'text-white' : 'text-slate-400'
                    )}
                  >
                    ...
                    {meta.slice(0, 5).map((m) => (
                      <tr key={m.strategy_key} className='hover:bg-white/5'>
                        <td className='py-2 text-slate-300 capitalize'>
                          {m.strategy_key.replace('_', ' ')}
                        </td>
                        <td
                          className={cn(
                            'py-2 text-right font-mono',
                            m.win_rate > 0.6 ? 'text-red-400' : 'text-cyan-400'
                          )}
                        >
                ...
                {mutations && mutations.length > 0 ? (
                  <div className='space-y-3'>
                    {mutations.map((mut) => (
                      <div
                        key={mut.id}
                        className='bg-yellow-950/20 border border-yellow-500/20 p-3 rounded text-sm mb-2'
                      >
                        <div className='flex justify-between items-center mb-1'>
                          <span className='text-yellow-200 font-bold capitalize'>
                            {mut.strategy_key.replace('_', ' ')}
                          </span>
                          <span
                            className={cn(
                              'text-xs font-bold px-1.5 py-0.5 rounded',
                              mut.mutation_suggestion.action === 'nerf'
            ...
            {actions.map((log) => (
              <div key={log.id} className='mb-2 flex gap-3 text-slate-400'>
                ...
                <span
                  className={cn(
                    'font-bold',
                    log.action_type === 'APPLY_MUTATION'
                      ? 'text-yellow-500'
                      : log.action_type === 'INTERVENE_PLAYER'
                        ? 'text-red-500'
                        : 'text-blue-500'
                  )}
                >
