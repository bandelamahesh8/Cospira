      ...
      <div className='w-full px-6 md:px-10 lg:px-12'>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            <Link to='/' className='flex items-center gap-3 group opacity-90 hover:opacity-100 transition-opacity'>
                <CospiraLogo size={32} className="text-zinc-100 group-hover:text-white transition-colors" />
                
                <span className={`font-bold tracking-tight text-white transition-all duration-300 text-sm md:text-base hidden md:block uppercase italic tracking-tighter`}>
                    Cospira
                </span>
            </Link>
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5 ml-2 md:ml-4 group/status relative">
                ...
                <div className="flex flex-col">
                    <div className="flex items-center gap-1.5">
                        <span className={`text-[8px] font-black uppercase tracking-widest ${isConnected ? 'text-emerald-400' : 'text-red-400'}`}>
                            {isConnected ? 'Signal Active' : 'Offline'}
                        </span>
                        <div className="h-1 w-1 rounded-full bg-white/20" />
                        <span className="text-[8px] font-black uppercase tracking-widest text-indigo-400">Live Ops</span>
                    </div>
                    <span className="text-[7px] text-white/30 font-bold uppercase tracking-[0.2em] hidden sm:block">Real-time Fetching</span>
                </div>
          ...
          <div className='flex items-center gap-4'>
             {/* Activity Pulse / Frequency */}
              {isConnected && (
                <div className="hidden lg:flex items-center gap-3">
                    ...
                    >
                        <Activity className={`w-3 h-3 ${isAiActive ? 'text-indigo-400' : 'text-emerald-400'} animate-pulse`} />
                        <span className={`text-[9px] font-black uppercase tracking-wider ${isAiActive ? 'text-indigo-400' : 'text-indigo-400/60'}`}>
            ...
            <div className="hidden md:flex items-center gap-6">
                {user ? <UserMenu /> : (
                    <Link to='/auth' className="text-[10px] font-bold uppercase tracking-widest text-white/60 hover:text-white transition-colors">
                        Sign In
                    </Link>
                )}
            </div>
          ...
          {isOpen && (
            ...
            >
                <div className="pt-4 pb-6 flex flex-col gap-2">
                    ...
                    {user ? (
                        <button onClick={signOut} className="flex items-center gap-2 px-4 py-3 text-red-400 font-bold text-xs uppercase tracking-widest hover:bg-white/5 rounded-xl transition-colors">
                            <LogOut className="w-4 h-4" /> Sign Out
                        </button>
                    ) : (
                        <Link to="/auth" className="block text-center mx-4 mt-2 px-4 py-3 bg-white text-black font-bold uppercase text-xs tracking-widest rounded-xl">
                            Sign In
                        </Link>
                    )}
                </div>
