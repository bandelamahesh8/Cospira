...
export const QuestWidget = () => {
    ...
    const handleClaim = async (quest: Quest) => {
        ...
        try {
            ...
            toast({ title: 'Reward Claimed', description: `+${quest.reward_amount} ${quest.reward_type}` });
            loadQuests();
        } catch (e) {
             console.error(e);
        }
                    ...
                    return (
                        <div key={q.id} className="bg-black/20 p-3 rounded-lg border border-white/5">
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="text-xs font-mono text-yellow-400">+{q.reward_amount} {q.reward_type.toUpperCase()}</div>
                            </div>
                            
                            <div className="flex items-center gap-3">
                                <div className="flex-1">
                                    <Progress value={percent} className="h-1.5 bg-black/50" indicatorClassName="bg-cyan-500" />
                                ...
                                {q.is_claimed ? (
                                    ...
                                    <Button size="sm" onClick={() => handleClaim(q)} className="h-6 text-[10px] bg-yellow-500 text-black hover:bg-yellow-600">
                                        CLAIM
                                    </Button>
                                ) : null}
                            </div>
