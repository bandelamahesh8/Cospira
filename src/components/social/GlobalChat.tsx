...
export const GlobalChat = () => {
    ...
    return (
        <div className="flex flex-col flex-1 w-full bg-[#030508]/95 backdrop-blur-xl overflow-hidden relative group">
            ...
            <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between bg-black/20">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-[9px] text-cyan-500/50 font-mono tracking-wider">SECURE CONNECTION</p>
                    </div>
                </div>
            ...
            {/* Messages Area */}
            <ScrollArea className="flex-1 p-4 bg-[url('/grid.svg')] bg-fixed bg-opacity-5" ref={scrollRef}>
                <div className="space-y-4">
                    {chatMessages.map((m) => {
                        const isMe = m.senderName === user?.user_metadata?.display_name || m.senderName === user?.email;
                        return (
                            <div key={m.id} className={cn("flex gap-3 items-start group/msg", isMe ? "flex-row-reverse" : "")}>
                                <Avatar className={cn("w-7 h-7 mt-0.5 border border-white/10 ring-2 ring-transparent transition-all", isMe ? "ring-cyan-500/20" : "group-hover/msg:ring-purple-500/20")}>
                                    <AvatarFallback className={cn("text-[9px] font-bold", isMe ? "bg-cyan-950 text-cyan-400" : "bg-slate-900 text-slate-400")}>
                                ...
                                <div className={cn(
                                    ...
                                    {!isMe && (
                                        <p className="font-bold text-[9px] text-purple-400 mb-1 uppercase tracking-wider flex items-center gap-1">
                                            {m.senderName}
                                            <span className="text-[8px] text-white/10 font-mono">#{m.senderId.slice(0,4)}</span>
                                        </p>
                                    )}
                                    ...
                                    {/* Corner Accent */}
                                    <div className={cn("absolute w-1.5 h-1.5", isMe ? "top-0 right-0 border-t border-r border-cyan-500/50" : "top-0 left-0 border-t border-l border-white/20")} />
                                </div>
                            </div>
                        );
                    })}
                    {chatMessages.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 opacity-30 gap-2">
                            ...
                            <p className="text-[10px] text-white font-mono uppercase tracking-widest">Channel Empty</p>
                        </div>
                    )}
                </div>
            </ScrollArea>
            ...
            {/* Input Area */}
            <form onSubmit={send} className="p-3 bg-black/40 border-t border-white/5 flex gap-2 backdrop-blur-md">
                <Input 
                    value={msg} 
                    ...
                    onFocus={playHover}
                />
                <Button size="icon" className="h-10 w-10 bg-cyan-600 hover:bg-cyan-500 text-white shadow-[0_0_15px_rgba(6,182,212,0.3)] transition-all hover:scale-105 border border-cyan-400/20">
