...
export const ClanManager = () => {
  ...
  const handleJoin = async (clanId: string) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to join. You might already be in a clan.',
        variant: 'destructive',
      });
    }
  };
  ...
  if (myClan) {
    return (
      <div className='p-8 max-w-4xl mx-auto text-white'>
        <div className='flex justify-between items-center mb-6'>
          <div>
            <h1 className='text-3xl font-bold flex items-center gap-3'>
              <Shield className='text-yellow-500 w-8 h-8' />[{myClan.tag}] {myClan.name}
            </h1>
        ...
        <ClanWarsList
          clanId={myClan.id}
          isLeader={myClan.members.find((m: any) => m.user_id === user.id)?.role === 'leader'}
        />
        ...
        <>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
            {allClans.map((clan) => (
              <Card
                key={clan.id}
                className='bg-slate-900 border-slate-700 p-5 hover:border-indigo-500/50 transition-all'
              >
                <div className='flex justify-between items-start mb-2'>
                  <h3 className='font-bold text-lg text-white'>
                    [{clan.tag}] {clan.name}
                  </h3>
                  <span className='text-xs bg-slate-800 px-2 py-1 rounded text-yellow-500 font-bold'>
                    Lvl {clan.level}
                  </span>
                </div>
    ...
    <div className='space-y-3'>
      {wars.map((war) => {
        ...
        return (
          <Card
            key={war.id}
            className='bg-slate-800/50 border-slate-700 p-4 flex justify-between items-center'
          >
            <div>
              <div className='flex items-center gap-2'>
                <span
                  className={`text-sm font-bold ${war.status === 'active' ? 'text-green-400' : 'text-slate-400'}`}
                >
              ...
              <div className='text-xs text-slate-400 mt-1'>
                Score: <span className='text-blue-400'>{war.challenger_score}</span> -{' '}
                <span className='text-red-400'>{war.defender_score}</span>
              </div>
            </div>
