      ...
      return (
        <div className='p-8 rounded-[2rem] bg-white/5 border border-white/10'>
          {rawSummary
            ...
            .filter((p) => p.trim())
            .map((p, i) => (
              <p key={i} className='text-base text-white/80 leading-relaxed mb-4 last:mb-0'>
                {p}
              </p>
            ))}
        </div>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
            {parsed.actionProtocol.map((item, i) => (
              <div key={i} className='flex gap-3 items-start group'>
                ...
                <p className='text-[13px] text-white/60 font-medium leading-relaxed group-hover:text-white transition-colors'>
                  {item}
                </p>
              </div>
            ))}
              ...
              <div className='flex items-center gap-6'>
                ...
                <div>
                  <h2 className='text-3xl font-black italic uppercase tracking-tighter text-white flex items-center gap-4'>
                    ...
                    <span className='px-3 py-1 rounded-lg bg-primary text-black text-[11px] font-black tracking-[0.2em] uppercase shadow-[0_0_20px_rgba(var(--primary-rgb),0.5)]'>
                      ELITE V4.0
                    </span>
                  </h2>
                ...
                {isLoading ? (
                  <div className='flex flex-col items-center justify-center py-24 space-y-8'>
                    ...
                    <div className='text-center space-y-3'>
                      <p className='text-2xl font-black text-white uppercase tracking-[0.4em]'>
                        Synthesizing Intel
                      </p>
                      <p className='text-xs text-white/20 font-black uppercase tracking-widest'>
                        Analyzing voice vectors, chat nodes, and activity metadata
                      </p>
                    </div>
                  </div>
                ) : data?.status === 'success' ? (
                  <div className='animate-in fade-in duration-1000'>
                    ...
                    <div className='mt-12 grid grid-cols-2 md:grid-cols-4 gap-4'>
                      {[
                        ...
                        <div
                          key={i}
                          className='p-6 rounded-3xl bg-white/[0.02] border border-white/5 flex flex-col items-center text-center group hover:bg-white/5 transition-all'
                        >
                          <span className='text-[9px] font-black text-white/20 uppercase tracking-[0.2em] mb-2 group-hover:text-primary/60 transition-colors'>
                            {stat.label}
                          </span>
                  ...
                  <div className='flex flex-col items-center justify-center py-24 text-center'>
                    ...
                    <p className='text-white/40 max-w-sm mx-auto mb-10 text-sm font-medium'>
                      Orpion AI requires at least 5 minutes of session baseline to establish
                      accurate neural mapping.
                    </p>
                  ...
                  <div className='flex flex-col items-center justify-center py-24 text-center'>
                    ...
                    <p className='text-white/40 max-w-md mx-auto mb-10 text-sm font-medium'>
                      {' '}
                      {data?.message ||
                        'Insufficient telemetry collected to generate a high-fidelity intelligence report.'}{' '}
                    </p>
                    <Button
                      onClick={fetchSummary}
                      variant='outline'
                      className='h-12 px-8 rounded-2xl border-white/10 hover:bg-white/5 text-white/50 hover:text-white transition-all font-black uppercase text-xs tracking-widest'
                    >
