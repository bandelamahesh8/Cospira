...
import { FileText, Sparkles, CheckCircle2, Info, X, Printer, Download, RefreshCw, Layers } from 'lucide-react';
...
const SummaryModal: React.FC<SummaryModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleGenerate = () => {
        ...
        socket.emit('ai:generate-summary', { roomId }, (response: { success: boolean; summary?: MeetingSummary; error?: string }) => {
...
`;
    ...
    const handleExportRaw = () => {
        const text = messages.map(m => `[${new Date(m.timestamp).toLocaleTimeString()}] ${m.userName}: ${m.content}`).join('\n');
        const blob = new Blob([text], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
                                ...
                                {!summary && !isGenerating && !error && (
                                    <div className="flex flex-col items-center justify-center py-20 text-center">
                                        ...
                                        <p className="text-white/40 max-w-md mb-8 leading-relaxed">
                                            Initiate the intelligence protocol to extract key decisions, action items, and summaries from the conversation logic.
                                        </p>
                                        <Button 
                                ...
                                {isGenerating && (
                                    <div className="flex flex-col items-center justify-center py-24 space-y-6">
                                        ...
                                        <div className="text-center">
                                            <p className="text-lg font-bold text-white tracking-wide">Processing Audio Data</p>
                                            <p className="text-xs font-mono text-indigo-400 mt-2 animate-pulse">Filtering relevant vectors...</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                {error && (
                                    <div className="bg-red-500/5 border border-red-500/20 p-8 rounded-3xl text-center max-w-lg mx-auto">
                                        ...
                                        <div className="flex justify-center gap-3">
                                            <Button variant="outline" onClick={handleExportRaw} className="bg-transparent border-white/10 hover:bg-white/5 text-white">Download Raw Logs</Button>
                                            <Button onClick={handleGenerate} className="bg-white text-black hover:bg-gray-200">Retry Protocol</Button>
                                        </div>
                                    </div>
                                )}
                                                ...
                                                <ul className="space-y-4">
                                                    {(summary.bullets || []).map((bullet: string, i: number) => (
                                                        <li key={i} className="flex gap-4 group">
                                                            ...
                                                            <span className="text-sm text-white/70 leading-relaxed group-hover:text-white transition-colors">{bullet}</span>
                                                        </li>
                                                    ))}
                                                    {(!summary.bullets || summary.bullets.length === 0) && (
                                                        <li className="text-sm text-white/30 italic pl-10">No key points extracted.</li>
                                                    )}
                                                </ul>
                                            </div>
                                                ...
                                                <div className="space-y-6">
                                                    ...
                                                    <div className="space-y-3">
                                                            ...
                                                            <div key={i} className="p-4 rounded-xl bg-white/5 border border-white/5 hover:border-white/10 transition-colors group">
                                                                <div className="flex items-start gap-3">
                                                                    <div className="mt-0.5 w-4 h-4 rounded border border-white/30 flex items-center justify-center group-hover:border-emerald-500 transition-colors">
                                                                        {item.status === 'completed' && <div className="w-2 h-2 bg-emerald-500 rounded-[1px]" />}
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-sm font-medium text-white/90 group-hover:text-white">{item.text}</p>
                                                                        {item.owner && (
                                                                            <p className="text-[10px] text-white/40 mt-1 uppercase tracking-wider">
                                                                                Assigned: <span className="text-emerald-400 font-bold">{item.owner}</span>
                                                                            </p>
                                                                        )}
                                                                    </div>
                                                                </div>
                                                        ...
                                                        {(!summary.actionItems || summary.actionItems.length === 0) && (
                                                            <p className="text-sm text-white/30 italic pl-2">No tasks assigned.</p>
                                                        )}
                                                    </div>
                                                </div>
                                                 ...
                                                 <div className="space-y-6">
                                                    ...
                                                    <div className="space-y-3">
                                                        {(summary.decisions || []).map((decision, i: number) => (
                                                            <div key={i} className="flex gap-3 items-start p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
                                                                <CheckCircle2 className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                                                                <p className="text-sm font-medium text-amber-200/90">{decision.decision}</p>
                                                            </div>
                                                        ))}
                                                        {(!summary.decisions || summary.decisions.length === 0) && (
                                                            <p className="text-sm text-white/30 italic pl-2">No decisions recorded.</p>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
