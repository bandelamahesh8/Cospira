            ...
            <div className='flex -space-x-3'>
              {users.slice(0, 3).map((u) => (
                <UserAvatar
                  ...
                  name={u.name}
                  seed={u.id}
                  className='w-6 h-6 border-2 border-[#0A0A0A]'
                />
              ))}
            </div>
    ...
    >
      ...
      <div className='flex items-center gap-3 overflow-x-auto no-scrollbar p-3 bg-[#0A0A0A]/60 backdrop-blur-2xl rounded-[1.5rem] border border-white/5 shadow-[0_20px_50px_rgba(0,0,0,0.5)]'>
        ...
        >
          ...
          <div className='absolute top-1 left-1 px-1.5 py-0.5 bg-primary/90 backdrop-blur-md text-background text-[8px] font-black uppercase rounded shadow-sm z-30'>
            You
          </div>
        </motion.div>

        {users
          .filter((u) => u.id !== (authUser?.id || socket?.id))
          .map((u) => {
            ...
            return (
              ...
              >
                ...
                <div className='absolute top-1 left-1 px-1.5 py-0.5 bg-black/60 backdrop-blur-md text-white/90 text-[8px] font-black uppercase rounded truncate max-w-[60px] z-30'>
                  {u.name}
                </div>
              </motion.div>
            );
