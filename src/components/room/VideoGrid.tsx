...
import { Pin, PinOff, ChevronLeft, ChevronRight, Wifi } from 'lucide-react';
...
const VideoGrid: React.FC<VideoGridProps> = ({
    ...
    const remoteUsers = users.filter(u => String(u.id) !== String(localUserId))
        .filter((user, index, self) => 
            index === self.findIndex(u => String(u.id) === String(user.id))
        );
    
    // Helper to render a tile
    const renderTile = (id: string, name: string, stream: MediaStream | null, isLocal: boolean, isMuted: boolean, photoUrl?: string | null, gender?: string, status?: 'online' | 'away', isVideoOn?: boolean, hideName?: boolean) => (
        <motion.div 
            layout
            layoutId={id} 
        ...
        >
            ...
            {!isSocialMode && (
                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all duration-300 flex gap-2 z-30 transform translate-y-2 group-hover:translate-y-0">
                    <button 
                        onClick={(e) => { e.stopPropagation(); setPinnedId(pinnedId === id ? null : id); }}
                ...
                <div className="flex-1 w-full min-h-0 relative rounded-3xl md:rounded-[2.5rem] overflow-hidden bg-[#151921] border border-white/10 shadow-2xl">
                    <AnimatePresence mode="wait">
                        {isSearching ? (
                                ...
                                <div className="relative flex flex-col items-center">
                                    <div className="relative mb-12">
                                        ...
                                        <div className="relative flex items-center justify-center">
                                            <div className="absolute w-[400px] h-[400px] border border-emerald-500/10 rounded-full animate-[spin_20s_linear_infinite]" style={{ borderStyle: 'dashed' }} />
                                            <div className="absolute w-[300px] h-[300px] border border-emerald-500/20 rounded-full animate-[spin_15s_linear_infinite_reverse]" style={{ borderStyle: 'dotted' }} />
                                    ...
                                    <div className="flex flex-col items-center gap-4 relative z-10">
                                         <h3 className="text-3xl font-black italic tracking-[-0.05em] text-white flex items-center gap-3">
                                            <span className="text-emerald-500 opacity-50 font-mono text-sm">SEC-LINK:</span>
                                         ...
                                         <div className="flex flex-col items-center gap-2">
                                            <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 px-4 py-1.5 rounded-full backdrop-blur-md">
                                                ...
                                                <span className="text-[10px] uppercase font-black tracking-[0.3em] text-emerald-500">Global Node Discovery Active</span>
                                            </div>
                                            <div className="text-[9px] text-white/20 font-mono tracking-widest uppercase">Encryption Mesh Stable • Searching for compatible signal</div>
                                         </div>
                                    </div>
                                </div>
                            </motion.div>
                            ...
                            <div key="waiting" className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-[#0F1115]">
                                ...
                                <div className="flex flex-col items-center gap-2 relative z-10">
                                    <span className="text-sm text-white/60 font-black uppercase tracking-[0.4em]">Establishing Uplink</span>
                                    <span className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Awaiting Remote Synchronicity</span>
                                </div>
    ...
    if (pinnedId) {
        ...
        const otherUsers = [
            ...(localUserId && pinnedId !== localUserId ? [{ 
                id: localUserId, 
                name: localUserName, 
                stream: localStream, 
                isLocal: true, 
            ...
            ...remoteUsers.filter(u => String(u.id) !== pinnedId).map(u => ({
                id: String(u.id),
                name: u.name,
                stream: remoteStreams.get(String(u.id)) || null,
                isLocal: false,
        ...
        return (
             <div className="w-full h-full flex flex-col md:flex-row gap-6 p-6 overflow-hidden">
                <div className="flex-1 h-[50vh] md:h-full rounded-[2.5rem] overflow-hidden relative shadow-[0_30px_60px_rgba(0,0,0,0.5)] border border-white/5 bg-[#0A0A0A]">
                    <AnimatePresence mode='wait'>
                        {focusedUser ? (
                            <motion.div 
                                key={focusedUser.id}
                                ...
                                animate={{ opacity: 1 }}
                                exit={{ opacity: 0 }}
                                layoutId={focusedUser.id}
                            >
                                {renderTile(focusedUser.id, focusedUser.name, focusedUser.stream, focusedUser.isLocal, focusedUser.isMuted, focusedUser.photoUrl, focusedUser.gender, focusedUser.status as 'online' | 'away' | undefined, focusedUser.isVideoOn, isSocialMode)}
                            </motion.div>
                        ) : (
                           <div className="flex items-center justify-center h-full text-white/30 font-mono text-sm uppercase">Signal Lost</div> 
                        )}
                    </AnimatePresence>
                </div>
                
                <div className="md:w-[280px] lg:w-[320px] flex-none overflow-y-auto custom-scrollbar pr-2">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-white/30 mb-4 pl-1">Participants</h3>
                    <div className="grid grid-cols-2 md:grid-cols-1 gap-4">
                        {otherUsers.map(u => (
                            <div key={u.id} className="aspect-video w-full">
                                {renderTile(u.id, u.name, u.stream, u.isLocal, u.isMuted, u.photoUrl, u.gender, u.status as 'online' | 'away' | undefined, u.isVideoOn, isSocialMode)}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            >
                <AnimatePresence mode='popLayout'>
                    {paginatedParticipants.map(u => (
                         <div key={u.id} className={`aspect-video w-full ${paginatedParticipants.length === 1 && totalPages === 1 ? 'aspect-[16/9] md:aspect-video h-full max-h-[80vh] shadow-2xl' : ''}`}>
                             {renderTile(u.id, u.name, u.stream, u.isLocal, u.isMuted, u.photoUrl, u.gender, u.status as 'online' | 'away' | undefined, u.isVideoOn, isSocialMode)}
                         </div>
                    ))}
                </AnimatePresence>
