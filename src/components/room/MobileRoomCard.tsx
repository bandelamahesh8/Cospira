...
export const MobileRoomCard = ({ room, onClick }: MobileRoomCardProps) => {
  const isAI =
    ...
    room.name.toLowerCase().includes('alpha') ||
    room.name.toLowerCase().includes('nebulon');
  const isGaming =
    room.mode === 'gaming' ||
    room.name.toLowerCase().includes('gaming') ||
    room.name.toLowerCase().includes('sigma') ||
    room.name.toLowerCase().includes('xenon') ||
    room.name.toLowerCase().includes('cyber');
      ...
      <div className='relative z-10 mt-6 pt-4 border-t border-white/5 flex items-center justify-between'>
        <div className='flex items-center gap-1'>
          {[1, 2, 3, 4, 5].map((s) => (
            <Star
              key={s}
              size={14}
              className={s <= 4 ? 'fill-amber-400 text-amber-400' : 'text-white/20'}
            />
          ))}
          ...
          <div className='flex items-center gap-3'>
            ...
            <div className='flex items-center gap-1.5 opacity-60'>
              ...
              <span className='text-[10px] font-black text-white'>
                {room.userCount - 5 > 0 ? room.userCount - 5 : 5}
              </span>
            </div>
          </div>
        </div>
        ...
        {room.name.includes('Nebulon') && (
          <div className='flex items-center gap-2'>
            <span className='px-2 py-0.5 rounded-full bg-indigo-500/20 border border-indigo-500/40 text-[8px] font-black text-indigo-400 uppercase tracking-widest'>
              Trending
            </span>
            <span className='px-2 py-0.5 rounded-full bg-rose-500/20 border border-rose-500/40 text-[8px] font-black text-rose-400 uppercase tracking-widest'>
              Live
            </span>
          </div>
        )}
