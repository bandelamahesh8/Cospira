...
export const RoomModeIndicator: React.FC<RoomModeIndicatorProps> = ({ roomId, isHost }) => {
  ...
  const TriggerButton = ({ isInteractive }: { isInteractive: boolean }) => (
    ...
    >
        ...
        {isInteractive && <div className="absolute inset-0 bg-gradient-to-r from-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />}
        
        <div className="relative z-10 flex items-center gap-3">
            <span className="text-lg filter drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]">{config.icon}</span>
            <div className="flex flex-col items-start gap-0.5">
                <span className="text-[9px] font-bold uppercase tracking-widest text-white/40">Current Protocol</span>
                <span className="text-xs font-black text-white uppercase tracking-wide flex items-center gap-1.5">
                    {config.label}
                    {isInteractive && <ChevronDown className="w-3 h-3 text-white/30 group-hover:text-white transition-colors" />}
                </span>
            </div>
        </div>
    </motion.button>
                ...
                <div className="flex items-start gap-3 relative z-10">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                        <span className={`text-xs font-bold uppercase tracking-wide ${isActive ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
                            {modeConfig.label}
                        </span>
                        {isActive && <div className="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(var(--primary),0.8)]" />}
                    </div>
                    <p className={`text-[10px] leading-relaxed line-clamp-2 ${isActive ? 'text-primary-foreground/80' : 'text-white/40 group-hover:text-white/60'}`}>
