...
export const SoftOnboarding = () => {
  const [currentHint, setCurrentHint] = useState<{ id: string, text: string, icon: React.ReactNode, type?: 'info' | 'pro' } | null>(null);
  ...
  useEffect(() => {
    ...
    if (hasSeenFirstTime(FirstTimeFlags.CONTROLS_HINT) && !hasSeenFirstTime(FirstTimeFlags.SHORTCUT_HINT)) {
        const timer = setTimeout(() => {
             setCurrentHint({ 
    ...
    <AnimatePresence>
      {currentHint && (
        ...
        >
            ...
            `}>
                {/* Glow Background */}
                <div className={`absolute inset-0 opacity-20 bg-gradient-to-r ${currentHint.type === 'pro' ? 'from-amber-500/20 to-orange-500/20' : 'from-indigo-500/20 to-purple-500/20'}`} />
                ...
                <div className="flex flex-col relative z-10">
                    <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${currentHint.type === 'pro' ? 'text-amber-500' : 'text-indigo-400'} mb-0.5 flex items-center gap-1`}>
                        {currentHint.type === 'pro' && <Sparkles className="w-3 h-3" />}
                        {currentHint.type === 'pro' ? 'Pro Insight' : 'System Guide'}
                    </span>
                    <span className="text-sm font-bold text-white tracking-wide font-sans">{currentHint.text}</span>
                </div>
