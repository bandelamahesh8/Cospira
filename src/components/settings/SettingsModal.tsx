...
import { Dialog, DialogContent, DialogOverlay, DialogTitle } from '@/components/ui/dialog'; // Using primitives for custom styling
import { Button } from '@/components/ui/button';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';
import { Slider } from '@/components/ui/slider';
import { Settings, Volume2, Eye, EyeOff, Bell, Cpu, Activity, Shield, Laptop, Zap, X } from 'lucide-react';
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogPrimitive.Portal>
                ...
                <DialogPrimitive.Content 
                    className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-2xl translate-x-[-50%] translate-y-[-50%] gap-0 border border-white/10 bg-[#05070a]/90 p-0 shadow-2xl duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] sm:rounded-3xl overflow-hidden"
                >
                     {/* Header */}
                    <div className="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-white/[0.02]">
                        <div className='flex items-center gap-4'>
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-indigo-400 font-mono uppercase tracking-widest">Version 2.4.0 // Build 9942</p>
                            </div>
                        </div>
                    ...
                    <div className="grid md:grid-cols-12 min-h-[500px] max-h-[80vh] md:max-h-[none] overflow-y-auto md:overflow-visible">
                        ...
                        <div className="md:col-span-4 bg-[#0a0d12] border-b md:border-b-0 md:border-r border-white/5 p-6 flex flex-col gap-6 relative overflow-hidden shrink-0">
                            ...
                            <div className="relative z-10 space-y-6">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/30 mb-4">Diagnostics</h3>
                                ...
                                <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 backdrop-blur-sm group hover:border-indigo-500/30 transition-colors">
                                    <div className="flex items-center justify-between mb-3">
                                        ...
                                        <span className={cn("text-xs font-mono", cpuLoad > 80 ? 'text-red-400' : 'text-indigo-400')}>{cpuLoad.toFixed(1)}%</span>
                                    </div>
                        ...
                        <div className="md:col-span-8 p-8 space-y-8 bg-gradient-to-br from-white/[0.01] to-transparent">
                            ...
                            <section className="space-y-4">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-sm font-black uppercase tracking-widest text-white/90">Audio Matrix</h3>
                                </div>

                                <div className="bg-white/5 border border-white/5 rounded-2xl p-6 hover:border-white/10 transition-colors">
                                    <div className="flex items-center justify-between mb-4">
                                        <Label className="text-xs font-bold uppercase tracking-wide text-white/60">Master Output Level</Label>
                                        <span className="px-2 py-1 rounded bg-black/40 border border-white/10 text-xs font-mono text-pink-400">{settings.volume}%</span>
                                    </div>
                                    <Slider 
                            ...
                            <section className="space-y-4">
                                <div className="flex items-center gap-3 mb-6">
                                    ...
                                    <h3 className="text-sm font-black uppercase tracking-widest text-white/90">Interface Protocol</h3>
                                </div>
                                ...
                                <div className="space-y-3">
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={cn("p-2 rounded-full transition-colors", settings.streamer_mode ? 'bg-purple-500/20 text-purple-400' : 'bg-white/5 text-white/30')}>
                                                {settings.streamer_mode ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                            </div>
                                            <div className="space-y-0.5">
                                                <Label className="text-sm font-bold text-white cursor-pointer">Streamer Mode</Label>
                                                <p className="text-[10px] text-white/40 font-medium">Obfuscate sensitive PII and user identities.</p>
                                            </div>
                                        </div>
                                        <Switch 
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className={cn("p-2 rounded-full transition-colors", settings.notifications ? 'bg-amber-500/20 text-amber-400' : 'bg-white/5 text-white/30')}>
                                                <Bell className="w-4 h-4" />
                                            </div>
                                            <div className="space-y-0.5">
                                                <Label className="text-sm font-bold text-white cursor-pointer">System Alerts</Label>
                                                <p className="text-[10px] text-white/40 font-medium">Receive heads-up display notifications.</p>
                                            </div>
                                        </div>
                                        <Switch 
