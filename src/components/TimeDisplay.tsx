    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button 
                    className="flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/5 rounded-full hover:bg-white/10 transition-colors group"
                >
                    ...
                    <span className="text-[8px] text-white/30 font-bold uppercase tracking-wider ml-1 hidden xl:inline-block">
                        {zone === 'local' ? 'LOC' : zone.includes('/') ? zone.split('/')[1].substring(0, 3).toUpperCase() : zone}
                    </span>
                </button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-48 bg-[#0A0C14] border-white/10 text-slate-200 backdrop-blur-xl">
