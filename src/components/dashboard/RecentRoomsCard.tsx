...
export const RecentRoomsCard = ({ title = 'Recent Rooms', subtitle = 'Your Active & Past Sessions', filterType }: RecentRoomsCardProps) => {
  const { user } = useAuth();
  ...
  return (
    <div className='bg-[#0F1116] border border-white/5 rounded-[24px] p-6 h-full flex flex-col relative overflow-hidden group/card'>
      ...
      <div className='flex items-center justify-between mb-6 relative z-10'>
        ...
        >
          {isMinimized ? <ChevronDown className='w-5 h-5 text-white' /> : <ChevronUp className='w-5 h-5 text-white' />}
        </button>
      </div>

      <div className={`transition-all duration-300 ease-in-out relative z-10 flex flex-col gap-3 ${isMinimized ? 'h-0 opacity-0 overflow-hidden' : 'h-auto opacity-100 flex-1'}`}>
        {!isMinimized && (
          <>
            {isLoading ? (
              <div className='flex-1 flex flex-col items-center justify-center gap-4 py-8'>
                <div className='w-6 h-6 border-b-2 border-indigo-500 rounded-full animate-spin' />
                <span className='text-[10px] text-zinc-500 uppercase tracking-widest font-bold'>Loading History...</span>
              </div>
                ...
                <div className='flex flex-col gap-3 overflow-y-auto max-h-[400px] custom-scrollbar pr-2'>
                  {recentRooms.map((room) => (
                    <NeuralInformer
                      ...
                      description={room.status === 'active' ? 'This room is currently active. Rejoin immediately.' : 'This session has expired, but you can try reconnecting.'}
                    >
                      <div
                      ...
                      >
                        <div className='flex items-center gap-4'>
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${room.status === 'active' ? 'bg-emerald-500/10' : 'bg-white/5'}`}>
                            <Activity className={`w-4 h-4 ${room.status === 'active' ? 'text-emerald-400' : 'text-zinc-500'}`} />
                          </div>
                        ...
                        <div className='flex items-center gap-3'>
                          <div className={`px-2 py-1 rounded border text-[9px] font-bold uppercase tracking-widest ${room.status === 'active' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.1)]' : 'bg-zinc-900 border-white/10 text-zinc-500'}`}>
              ...
              <div className='flex-1 flex flex-col items-center justify-center text-center px-4 py-8 bg-black/20 rounded-xl border border-white/5 border-dashed'>
                ...
                <span className='text-xs font-bold text-zinc-500 uppercase tracking-widest'>No History Found</span>
                <span className='text-[10px] text-zinc-600 mt-1 max-w-[200px] leading-relaxed'>Your recent room connections will appear here.</span>
              </div>
            )}
          </>
        )}
