...
export const CreateRoomPanel = ({ onCreate, isCreating }: CreateRoomPanelProps) => {
  ...
  const handleCreate = () => {
    ...
    if ((privacy === 'encrypted' || mode === 'ultra') && !password.trim()) {
      if (mode === 'ultra') {
        toast.error('Security Protocol Incomplete', { description: 'Ultra Security requires a mandatory passkey.' });
      } else {
        toast.error('Security Protocol Incomplete', { description: 'Passkey required for encrypted sectors.' });
      }
      return;
    }
    onCreate({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <Label className="text-[10px] font-black uppercase tracking-widest text-white/30 px-2">Privacy Access</Label>
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <div className={`p-2 rounded-xl ${privacy === 'public' ? 'bg-emerald-500/20 text-emerald-400 font-black' : 'bg-white/5 text-white/40'}`}>
              <Globe className="w-5 h-5 font-black" />
            </div>
            <div className="text-left">
              <div className={`text-xs font-black uppercase tracking-tight ${privacy === 'public' ? 'text-white' : 'text-white/60'}`}>Public Sector</div>
              <div className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Open Uplink</div>
            </div>
          </button>
          ...
          >
            <div className={`p-2 rounded-xl ${privacy === 'encrypted' ? 'bg-indigo-500/20 text-indigo-400 font-black' : 'bg-white/5 text-white/40'}`}>
              <Shield className="w-5 h-5 font-black" />
            </div>
            <div className="text-left">
              <div className={`text-xs font-black uppercase tracking-tight ${privacy === 'encrypted' ? 'text-white' : 'text-white/60'}`}>Encrypted</div>
              <div className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Passkey Locked</div>
            </div>
          </button>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <Label className="text-[10px] font-black uppercase tracking-widest text-white/30 px-2">Configuration</Label>
        <div className="flex gap-2">
          <div className={`transition-all duration-500 ${privacy === 'encrypted' ? 'flex-[1.5]' : 'w-full'}`}>
            <Input 
              value={roomName}
      ...
      {/* Room Mode Selection */}
      <div className="space-y-2">
        <Label className="text-[10px] font-black uppercase tracking-widest text-white/30 px-2">Sector Protocol</Label>
        <TooltipProvider delayDuration={0}>
          <div className="grid grid-cols-4 gap-2">
            {[
              { id: 'fun', icon: Gamepad2, label: 'Fun', color: 'text-purple-400', bg: 'bg-purple-500/10', border: 'border-purple-500/20' },
              { id: 'professional', icon: Briefcase, label: 'Pro', color: 'text-cyan-400', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20' },
              { id: 'ultra', icon: Lock, label: 'Ultra', color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20' },
              { id: 'mixed', icon: Shuffle, label: 'Mixed', color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20' }
            ].map((m) => (
              <Tooltip key={m.id}>
                <TooltipTrigger asChild>
                  ...
                  >
                    ...
                    <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${mode === m.id ? 'text-white' : 'text-white/30'}`}>{m.label}</span>
                  </button>
                </TooltipTrigger>
                <TooltipContent side="bottom" className="bg-[#1A1F2B] text-white border-white/10 p-4 max-w-[240px] rounded-2xl shadow-2xl">
                  <p className="text-xs font-bold uppercase tracking-widest text-indigo-400 mb-1">{m.label} Mode</p>
                  <p className="text-[10px] font-medium text-white/60 leading-relaxed uppercase tracking-tight">{MODE_DESCRIPTIONS[m.id as RoomMode]}</p>
                </TooltipContent>
              </Tooltip>
            ))}
          </div>
      ...
      <div className="pt-2">
        ...
        >
          {isCreating ? 'Deploying...' : <> Initiate Protocol <ArrowRight className="w-4 h-4" /> </>}
        </button>
      </div>
    </div>
  );
