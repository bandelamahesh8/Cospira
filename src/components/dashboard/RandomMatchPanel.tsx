                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-white/30 ml-1">Interface Priority</label>
                                    <div className="grid grid-cols-2 gap-4 h-14">
                                        ...
                                        >
                                            <Video className={`w-4 h-4 ${mode === 'video' ? 'text-indigo-400' : 'text-white/20'}`} />
                                            <span className={`text-xs font-black uppercase tracking-wider ${mode === 'video' ? 'text-white' : 'text-white/40'}`}>Video</span>
                                            {mode === 'video' && (
                                        ...
                                        >
                                            <MessageSquare className={`w-4 h-4 ${mode === 'chat' ? 'text-indigo-400' : 'text-white/20'}`} />
                                            <span className={`text-xs font-black uppercase tracking-wider ${mode === 'chat' ? 'text-white' : 'text-white/40'}`}>Chat</span>
                                            {mode === 'chat' && (
                                                <div className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-indigo-500 shadow-[0_0_5px_currentColor]" />
                                            )}
                                        </button>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-white/30 ml-1">Common Interests (Optional)</label>
                                    <div className="h-14 bg-[#11141A] border border-white/5 rounded-xl flex items-center px-4 relative focus-within:border-white/20 transition-colors">
                                        <div className="flex gap-2 mr-2 overflow-x-auto no-scrollbar max-w-[60%]">
                                            {interests.map((tag, i) => (
                                                <span key={i} className="shrink-0 h-6 px-2 bg-white/10 rounded flex items-center gap-1 text-[10px] font-bold uppercase text-white cursor-pointer hover:bg-white/20" onClick={() => removeInterest(i)}>
                                                    {tag} <X size={10} />
                                                </span>
                                            ))}
                                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-widest text-white/30 ml-1">Connect Intent</label>
                                    <div className="grid grid-cols-4 gap-3 h-20">
                                        {[
                                            { id: 'casual', icon: Coffee, label: 'Casual', color: 'text-emerald-400', activeBg: 'bg-emerald-500/10', border: 'border-emerald-500/50' },
                                            { id: 'focus', icon: BookOpen, label: 'Focus', color: 'text-amber-400', activeBg: 'bg-amber-500/10', border: 'border-amber-500/50' },
                                            { id: 'network', icon: Users, label: 'Network', color: 'text-blue-400', activeBg: 'bg-blue-500/10', border: 'border-blue-500/50' },
                                            { id: 'play', icon: Gamepad2, label: 'Play', color: 'text-fuchsia-400', activeBg: 'bg-fuchsia-500/10', border: 'border-fuchsia-500/50' }
                                        ].map((item) => (
                                            <button
                                                key={item.id}
                                            ...
                                            >
                                                <item.icon className={`w-5 h-5 ${intent === item.id ? item.color : 'text-white/20'}`} />
                                                <span className={`text-[9px] font-bold uppercase tracking-widest ${intent === item.id ? 'text-white' : 'text-white/30'}`}>{item.label}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                        ...
                        >
                            ...
                            <div className="w-full md:w-1/2 aspect-video bg-black rounded-xl overflow-hidden relative border border-white/10 shadow-2xl">
                                {isChecking ? (
                                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0F1219]">
                                        ...
                                        <p className="text-white/40 font-mono text-xs uppercase tracking-widest">Verifying Hardware...</p>
                                    </div>
                                ) : permissionGranted ? (
                                    mode === 'video' ? (
                                        <video ref={previewVideoRef} autoPlay muted playsInline className="w-full h-full object-cover transform scale-x-[-1]" />
                                    ) : (
                                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#0F1219]">
                                            ...
                                            <div className="mt-4 flex gap-1 h-4 items-end">
                                                {[...Array(5)].map((_, i) => (
                                                    <div key={i} className="w-1 bg-indigo-500 animate-pulse rounded-full" style={{ height: Math.random() * 16 + 4, animationDelay: `${i * 0.1}s` }} />
                                                ))}
                                            </div>
                                        </div>
                                    )
                                ...
                                <div className="absolute top-3 right-3 px-2 py-1 bg-black/60 rounded backdrop-blur-md border border-white/10 flex items-center gap-2">
                                    <div className={`w-2 h-2 rounded-full ${permissionGranted ? 'bg-emerald-500' : 'bg-red-500'}`} />
                                    <span className="text-[10px] font-bold uppercase text-white/60">{permissionGranted ? 'SYSTEMS NOMINAL' : 'CHECKING...'}</span>
                                </div>
                            </div>
                            ...
                            <div className="w-full md:w-1/2 space-y-6">
                                <div className="space-y-4">
                                    ...
                                    >
                                        <div className={`mt-1 p-1 rounded border ${ageConfirmed ? 'bg-indigo-500 border-indigo-500 text-white' : 'border-white/20 text-transparent'}`}>
                                            <CheckCircle size={12} fill="currentColor" />
                                        </div>
                                        <div>
                                            <p className="text-sm font-bold text-white mb-1 group-hover:text-indigo-400 transition-colors">I accept the Terms of Engagement</p>
                                            <p className="text-xs text-white/40 leading-relaxed">By proceeding, I verify that I am 18+ years of age and agree to follow the community safety guidelines. Inappropriate behavior results in an immediate ban.</p>
                                        </div>
                                    </div>
                                </div>
