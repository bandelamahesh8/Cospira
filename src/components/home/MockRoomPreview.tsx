            ...
            <div className="flex-1 flex flex-col gap-4">
                ...
                <div className="h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center gap-4 px-6 relative overflow-hidden">
                    ...
                    {[Mic, Video, Monitor, MessageSquare, Users].map((Icon, i) => (
                        <div key={i} className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-white/20 hover:scale-110 transition-all cursor-default">
                ...
                <div className="flex-1 flex flex-col gap-2 overflow-hidden">
                    {[1, 2, 3].map((i) => (
                        <div key={i} className={`flex flex-col gap-1 ${i % 2 === 0 ? 'items-end' : 'items-start'}`}>
                            <div className={`px-3 py-2 rounded-xl text-[10px] text-white/80 max-w-[80%] ${i % 2 === 0 ? 'bg-primary/20 rounded-tr-none' : 'bg-white/10 rounded-tl-none'}`}>
