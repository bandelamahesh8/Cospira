...
const VideoTile: React.FC<VideoTileProps> = ({
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      logger.debug(
        `[VideoTile] Metadata loaded for ${username}, size: ${videoEl.videoWidth}x${videoEl.videoHeight}`
      );
  ...
  return (
    ...
    >
          ...
          >
            ...
            <div className='relative'>
              ...
              <div className='relative z-20 shadow-[0_20px_50px_rgba(0,0,0,0.5)]'>
                <UserAvatar
                  ...
                  className={
                    hideName
                      ? 'h-12 w-12 border-2 border-[#0A0A0A] shadow-xl'
                      : 'h-28 w-28 md:h-32 md:w-32 border-4 border-[#0A0A0A] shadow-2xl'
                  }
                />

                {/* Status Indicator on Avatar */}
                <div
                  className={`absolute bottom-0.5 right-0.5 rounded-full border-[2px] border-[#0A0A0A] flex items-center justify-center ${
                    isMuted ? 'bg-red-500' : 'bg-green-500'
                  } ${hideName ? 'w-4 h-4' : 'w-6 h-6'}`}
                >
                  {isMuted ? (
                    <MicOff className={hideName ? 'w-2 h-2 text-white' : 'w-3 h-3 text-white'} />
                  ) : (
                    <Mic className={hideName ? 'w-2 h-2 text-white' : 'w-3 h-3 text-white'} />
                  )}
                </div>
              </div>
            </div>
            ...
            <div className='mt-6 flex flex-col items-center gap-1'>
              ...
              <div className='flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/5 rounded-full backdrop-blur-md'>
                <div
                  className={`w-1.5 h-1.5 rounded-full ${isMuted ? 'bg-red-500/50' : 'bg-emerald-500 animate-pulse'}`}
                />
        ...
        <div className='absolute inset-0 z-20 flex flex-col justify-between p-4 bg-gradient-to-b from-black/60 via-transparent to-black/80 opacity-0 hover:opacity-100 transition-opacity duration-300'>
          <div className='flex justify-between items-start'>
            <div className='flex px-3 py-1 bg-black/40 backdrop-blur-xl rounded-full border border-white/10 items-center gap-2'>
              <div
                className={`w-2 h-2 rounded-full ${isMuted ? 'bg-red-500' : 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]'}`}
              />
      ...
      {/* Always visible nameplate for video state (minimized) - only show in full tile, not thumbnail */}
      {!hideName && (
        <div className='absolute bottom-4 left-4 z-20 pointer-events-none'>
          <div className='flex flex-col'>
            <h3 className='text-lg font-black text-white uppercase tracking-tight shadow-black drop-shadow-lg'>
              {username}
            </h3>
            {hasVideo && (
