...
export const guardSound = (volume: number): boolean => {
  if (volume > XO_CONFIG.SOUND.MAX_VOLUME) {
    console.error(
      `[XO Guard] Sound rejected: volume ${volume} exceeds max ${XO_CONFIG.SOUND.MAX_VOLUME}`
    );
    return false;
  }
  return true;
...
export const guardUIElement = (currentCount: number, adding: boolean = true): boolean => {
  if (adding && currentCount >= XO_CONFIG.DESIGN_LOCK.MAX_UI_ELEMENTS) {
    console.error(
      `[XO Guard] UI element rejected: max ${XO_CONFIG.DESIGN_LOCK.MAX_UI_ELEMENTS} elements`
    );
...
export const guardNoiseBudget = (event: {
  ...
  if (sounds > XO_CONFIG.NOISE_BUDGET.MAX_SOUNDS_PER_EVENT) {
    console.error(
      `[XO Guard] Event rejected: ${sounds} sounds exceeds max ${XO_CONFIG.NOISE_BUDGET.MAX_SOUNDS_PER_EVENT}`
    );
    return false;
  }
  ...
  if (animations > XO_CONFIG.NOISE_BUDGET.MAX_ANIMATIONS_PER_EVENT) {
    console.error(
      `[XO Guard] Event rejected: ${animations} animations exceeds max ${XO_CONFIG.NOISE_BUDGET.MAX_ANIMATIONS_PER_EVENT}`
    );
    return false;
  }
  ...
  if (stateChanges > XO_CONFIG.NOISE_BUDGET.MAX_STATE_CHANGES_PER_TURN) {
    console.error(
      `[XO Guard] Event rejected: ${stateChanges} state changes exceeds max ${XO_CONFIG.NOISE_BUDGET.MAX_STATE_CHANGES_PER_TURN}`
    );
    return false;
  }
